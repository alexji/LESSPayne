...
from lesspayne.smh.renamed_loaders import renamed_load
...
from .optimize_stellar_params import (
    optimize_stellar_parameters as run_optimize_stellar_parameters, )
...
class Session(BaseSession):
    ...
    def __init__(self, spectrum_paths, twd=None, from_load=False, **kwargs):
        ...
        common_metadata_keys = ["RA", "DEC", "OBJECT"] + kwargs.pop(
        ...
        if not from_load:
            ...
            if line_list_filename is not None and os.path.exists(
                    line_list_filename):
                self.import_linelist_as_profile_models(line_list_filename)

            # Construct any default spectral models.
            deconstructed_spectral_models = self.setting(
                ("default_spectral_models", ))
            if deconstructed_spectral_models is not None:
                logger.warn(
                    "default_spectral_models is not implemented (skipping)")
    ...
    def save(self, session_path, overwrite=False, **kwargs):
        ...
        def safe_path(path, twd, metadata):
            ...
            existing_paths = np.hstack(
                list(metadata["reconstruct_paths"].values()))
        ...
        if "template_spectrum_path" in metadata.get(
                "rv", {}) and os.path.exists(
                    metadata["rv"]["template_spectrum_path"]):
            twd_paths.append(
                safe_path(metadata["rv"].pop("template_spectrum_path"), twd,
                          metadata))
            metadata["reconstruct_paths"][
                "template_spectrum_path"] = os.path.basename(twd_paths[-1])

        # Line list.
        if "line_list" in metadata:
            raise IOError(
                "This is an old session (version<0.2) with line_list (NOT SAVING)!"
        ...
        twd_path = safe_path(os.path.join(twd, "normalized_spectrum.txt"), twd,
                             metadata)
        try:
            ...
            metadata["reconstruct_paths"][
                "normalized_spectrum"] = os.path.basename(twd_path)
        ...
        for path in twd_paths:
            try:
                ...
                logger.warn("Cannot save path '{}' to session:".format(path))

                logger.warn("Continuing to save session without this path "
        ...
        tarball.close()
        logger.info("Saved file to {} ({:.1f}s)".format(
            session_path,
            time.time() - start0))
        ...
        if exception_occurred:
            logger.exception(
                "Paths unable to save to session: {}".format(failed_paths))
            raise
    ...
    def import_spectral_model_states(self, path):
        ...
        with open(path, "rb") as fp:
            spectral_model_states = renamed_load(fp, encoding="latin1")
        spectral_models = self.reconstruct_spectral_models(
    ...
    def reconstruct_spectral_models(self, spectral_model_states):
        ...
        for state in spectral_model_states:
            ...
            if state["type"] == "SpectralSynthesisModel":
                ...
                raise ValueError(
                    "unrecognized spectral model class '{}'".format(
                        state["type"]))
            model = klass(*args)
            ## python 2/3 issue
            if "rt_abundances" in state["metadata"].keys():
                state["metadata"]["rt_abundances"] = utils._fix_bytes_dict(
                    state["metadata"]["rt_abundances"])
            model.metadata.update(state["metadata"])
            reconstructed_spectral_models.append(model)
            t2 = time.time() - start
            if t2 > 1.0:
                logger.debug(
                    "  Long time to load model {:.3f} {} {} {}\n".format(
                        t2, len(model.transitions), model.elements,
                        model.wavelength))

        return reconstructed_spectral_models

    def export_spectral_models(self,
                               path,
                               overwrite=False,
    ...
    def load(cls, session_path, skip_spectral_models=False, **kwargs):
        ...
        session.metadata["stellar_parameters"].setdefault(
            "syserr_effective_temperature", 150)
        session.metadata["stellar_parameters"].setdefault(
            "syserr_surface_gravity", 0.3)
        session.metadata["stellar_parameters"].setdefault(
            "syserr_metallicity", 0.0)
        session.metadata["stellar_parameters"].setdefault(
            "syserr_microturbulence", 0.2)
        session.metadata["stellar_parameters"].setdefault(
            "staterr_effective_temperature", 10)
        session.metadata["stellar_parameters"].setdefault(
            "staterr_surface_gravity", 0.1)
        session.metadata["stellar_parameters"].setdefault(
            "staterr_metallicity", 0.1)
        session.metadata["stellar_parameters"].setdefault(
            "staterr_microturbulence", 0.1)
        ...
        logger.debug("Time to reconstruct {} spectral models: {:.3f}".format(
            len(reconstructed_spectral_models),
            time.time() - start))
        ...
        logger.info("Loaded file {} ({:.1f}s)".format(session_path,
                                                      time.time() - start0))
        logger.debug("Input spectra paths: {}".format(
            session._input_spectra_paths))

        return session

    def apply_spectral_model_quality_constraints(self,
                                                 constraints,
                                                 only=None,
                                                 full_output=False):
        """
        ...
        for i, spectral_model in enumerate(self.metadata["spectral_models"]):
            if (not only(spectral_model) or not spectral_model.is_acceptable
                    or spectral_model.is_upper_limit):
                continue
    ...
    def _get_overlap_order(self, wavelength_regions, template_spectrum=None):
        ...
        for wl_start, wl_end in wavelength_regions:
            ...
            if template_spectrum is not None and not (
                    wl_start > template_spectrum.dispersion[0]
                    and wl_end < template_spectrum.dispersion[-1]):
                continue

            # Do any observed orders cover any part of this range?
            overlaps, indices = specutils.find_overlaps(self.input_spectra,
                                                        (wl_start, wl_end),
                                                        return_indices=True)
            if not overlaps:
                continue
            ...
            raise ValueError(
                "no wavelength regions are common to the template "
    ...
    def rv_measure(
        ...
        **kwargs,
        ...
        overlap_order, overlap_index, wavelength_region = self._get_overlap_order(
            wavelength_region, template_spectrum)
    ...
    def rv_correct(self, rv):
        ...
        try:
            ...
            print(
                "Failure to read FITS headers, will not have heliocentric/barycentric corrections"
            )
    ...
    def stitch_and_stack(self, **kwargs):
        normalized_orders = []
        for i, (spectrum, continuum) in enumerate(
                zip(self.input_spectra,
    ...
    def stellar_parameters(self):
        ...
        """
        return (
            self.metadata["stellar_parameters"]["effective_temperature"],
    ...
    def stellar_parameters_staterr(self):
        ...
        """
        return (
            self.metadata["stellar_parameters"]
            ["staterr_effective_temperature"],
    ...
    def set_stellar_parameters_errors(self, sysstat, dTeff, dlogg, dvt, dMH):
        ...
        self.metadata["stellar_parameters"][sysstat +
                                            "err_effective_temperature"] = (
                                                dTeff)
        self.metadata["stellar_parameters"][sysstat +
                                            "err_surface_gravity"] = dlogg
        self.metadata["stellar_parameters"][sysstat +
                                            "err_microturbulence"] = dvt
        self.metadata["stellar_parameters"][sysstat + "err_metallicity"] = dMH
        return None

    def stellar_parameter_uncertainty_analysis(
        self,
        transitions=None,
        ...
        if transitions is None:
            ...
            for model in self.metadata["spectral_models"]:
                if (model.is_acceptable
                        and isinstance(model, ProfileFittingModel)
                        and model.elements[0] == "Fe"):
            ...
            assert np.all(eqws > 0.01), np.sum(eqws <= 0.01)
            abundances = self.rt.abundance_cog(self.stellar_photosphere,
                                               transitions,
                                               twd=self.twd)
            ...
            assert np.all(eqws > 0.01), np.sum(eqws <= 0.01)
            for needed_col in [
                    "abundance", "expot", "reduced_equivalent_width"
            ]:
                assert needed_col in transitions.colnames, needed_col
        initial_slopes = utils.equilibrium_state(
            transitions,
        ...
        try:
            ...
            self.metadata["stellar_parameters"] = deepcopy(
                saved_stellar_params)
            abundances = self.rt.abundance_cog(self.stellar_photosphere,
                                               transitions,
                                               twd=self.twd)
            ...
            try:
                self.metadata["stellar_parameters"] = deepcopy(
                    saved_stellar_params)
                m, b, median, sigma, N = initial_slopes[
                    expot_balance_species].get(
                        "expot", (np.nan, np.nan, np.nan, np.nan, 0))
                logger.info(
                    "Finding error in Teff slope: {:.3f} +/- {:.3f}".format(
                        m, sigma[1]))
                ...
                def _calculate_teff_slope(teff):
                    ...
                    abundances = self.rt.abundance_cog(
                        self.stellar_photosphere, transitions, twd=self.twd)
                    transitions["abundance"] = abundances
                    m, b, median, sigma, N = utils.equilibrium_state(
                        transitions,
                        columns=("expot", ),
                        ycolumn="abundance",
                        yerr_column=yerr_column,
                    )[expot_balance_species]["expot"]
                    logger.debug("Teff={:.0f} m={:.3f} m_target={:.3f}".format(
                        teff, m, m_target))
                    return m

                minfn = lambda teff: (m_target - _calculate_teff_slope(teff[0])
                                      )**2
                Teff_positive_slope = fmin(minfn, [Teff],
                                           xtol=tolerances[0],
                                           ftol=0.00001)[0]
            ...
            try:
                ...
                def _get_fe_values(abundances, transitions):
                    ii1 = transitions[
                        "species"] == ionization_balance_species_1
                    ii2 = transitions[
                        "species"] == ionization_balance_species_2
                    N1 = np.sum(ii1)
                    N2 = np.sum(ii2)
                    ab1 = np.mean(abundances[ii1])
                    ab2 = np.mean(abundances[ii2])
                ...
                abundances = self.rt.abundance_cog(self.stellar_photosphere,
                                                   transitions,
                                                   twd=self.twd)
                abFe1, abFe2, semFe1, semFe2 = _get_fe_values(
                    abundances, transitions)
                dFe0 = abFe1 - abFe2
                logger.debug(
                    "Finding error in logg: Fe1={:.2f}+/-{:.2f}, Fe2={:.2f}+/-{:.2f}"
                    .format(abFe1, semFe1, abFe2, semFe2))
                ...
                def _calculate_logg_dFe(logg):
                    ...
                    abundances = self.rt.abundance_cog(
                        self.stellar_photosphere, transitions, twd=self.twd)
                    ab1, ab2, sem1, sem2 = _get_fe_values(
                        abundances, transitions)
                    logger.debug(
                        "logg={:.2f} dFe={:.3f} dFe_target={:.3f}".format(
                            logg, ab1 - ab2, dFe_target))
                    return ab1 - ab2

                minfn = lambda logg: (dFe_target - _calculate_logg_dFe(logg[0])
                                      )**2
                logg_positive_error = fmin(minfn, [logg],
                                           xtol=tolerances[1],
                                           ftol=0.00001)[0]
            ...
            try:
                self.metadata["stellar_parameters"] = deepcopy(
                    saved_stellar_params)
                m, b, median, sigma, N = initial_slopes[
                    rew_balance_species].get(
                        "reduced_equivalent_width",
                        (np.nan, np.nan, np.nan, np.nan, 0))
                logger.info(
                    "Finding error in vt slope: {:.3f} +/- {:.3f}".format(
                        m, sigma[1]))
                ...
                def _calculate_vt_slope(vt):
                    ...
                    m, b, median, sigma, N = utils.equilibrium_state(
                        transitions,
                        columns=("reduced_equivalent_width", ),
                        ycolumn="abundance",
                        yerr_column=yerr_column,
                    )[rew_balance_species]["reduced_equivalent_width"]
                    logger.debug("vt={:.2f} m={:.3f} m_target={:.3f}".format(
                        vt, m, m_target))
                    return m

                minfn = lambda vt: (m_target - _calculate_vt_slope(vt[0]))**2
                vt_positive_slope = fmin(minfn, [vt],
                                         xtol=tolerances[2],
                                         ftol=0.00001)[0]
            ...
            self.metadata["stellar_parameters"] = deepcopy(
                saved_stellar_params)
            self.set_stellar_parameters_errors("stat", Teff_error, logg_error,
                                               vt_error, mh_error)
            logger.info(
                "Uncertainties: dTeff={:.0f} dlogg={:.2f} dvt={:.2f} dmh={:.2f}, took {:.1f}s"
                .format(Teff_error, logg_error, vt_error, mh_error,
                        time.time() - start))
            if systematic_errors is not None:
                if len(systematic_errors) != 4:
                    logger.warn(
                        "Systematic errors are not length 4! {}".format(
                            systematic_errors))
                    logger.warn("Skipping...")
                else:
                    self.set_stellar_parameters_errors(
                        "sys",
                        systematic_errors[0],
                        systematic_errors[1],
                        systematic_errors[2],
    ...
    def compute_all_abundance_uncertainties(self, print_memory_usage=False):
        ...
        logger.info("Finished abundance uncertainty loop in {:.1f}s".format(
            time.time() - start))
        return data

    def propagate_stellar_parameter_error_to_equivalent_widths(
        self,
        Teff_error,
        logg_error,
        vt_error,
        ...
        for model in self.spectral_models:
            if (model.is_acceptable and isinstance(model, ProfileFittingModel)
                    and (not model.is_upper_limit)):
        ...
        abund0 = self.measure_abundances(eqw_models,
        ...
        error_output = {}
        error_output["stellar_parameter_errors"] = [
            Teff_error,
            logg_error,
            vt_error,
            mh_error,
        ...
        for spname, error in zip(spnames,
                                 [Teff_error, logg_error, vt_error, mh_error]):
            sperr_list = []
            for sign in [+1.0, -1.0]:
                abund_diffs = {}
                ...
                logger.debug(spname + " " +
                             str(self.metadata["stellar_parameters"][spname]))
                self.measure_abundances(eqw_models,
                                        save_abundances=True,
                                        calculate_uncertainties=False)
                summary = self.summarize_spectral_models()
                for species in all_species:
                    logger.debug("{:4.1f} {:5.2f} {:5.2f}".format(
                        species, summary[species][1], summary0[species][1]))
                    abund_diffs[
                        species] = summary[species][1] - summary0[species][1]
                for ratio in all_ratios:
                    abund_diffs[ratio] = (
                        summary[ratio[0]][1] -
                        summary[ratio[1]][1]) - ratios0[ratio]
            ...
            def simplify_sperr_output(error_output):
                ...
                for i, species in enumerate(all_species):
                    ...
                    for j0, spname in enumerate(spnames):
                        ...
                        for k0, (sign,
                                 sperr) in enumerate(zip([+1, -1],
                                                         sperr_list)):
                            icol = 1 + 2 * j0 + k0
                            data[i, icol] = sperr[species]
                        data[i, 9] += (np.max(
                            np.abs([data[i, 1 + 2 * j0],
                                    data[i, 2 * j0 + 1]])))**2
    ...
    def stellar_parameter_state(self, full_output=False, **kwargs):
        ...
        if len(ews) == 0 or np.isfinite(ews).sum() == 0:
        ...
        finite = np.logical_and(
            np.isfinite(transitions["equivalent_width"]),
        ...
        for index, abundance in zip(spectral_model_indices[finite],
                                    abundances):
            self.metadata["spectral_models"][int(
                index)].metadata["fitted_result"][-1]["abundances"] = [
                    abundance
                ]
        ...
        propagated_abundances = self.rt.abundance_cog(self.stellar_photosphere,
                                                      _transitions[finite],
                                                      twd=self.twd)

        for index, abundance, propagated_abundance in zip(
                spectral_model_indices[finite],
                transitions["abundance"][finite],
                propagated_abundances,
        ):
            self.metadata["spectral_models"][int(index)].metadata[
                "fitted_result"][-1]["abundance_uncertainties"] = [
                    propagated_abundance - abundance
                ]

        transitions["abundance_uncertainty"] = np.nan * np.ones(
            len(transitions))
        ...
        transitions["abundance_uncertainty"][finite] = np.clip(
            transitions["abundance_uncertainty"][finite], 0.005, np.inf)
        ...
        transitions["reduced_equivalent_width"] = np.log10(
            1e-3 * transitions["equivalent_width"] / transitions["wavelength"])
                ...
                if spectral_model.is_acceptable:
                    try:
                        equivalent_widths.append(
                            1000.0 * spectral_model.metadata["fitted_result"]
                            [-1]["equivalent_width"][0])
                    except:
                        equivalent_widths.append(np.nan)
                    try:
                        equivalent_width_errs.append(
                            1000.0 *
                            np.nanmax(spectral_model.metadata["fitted_result"]
                                      [-1]["equivalent_width"][1:3]))
                    except:
                        equivalent_width_errs.append(np.nan)
                    try:
                        abundances.append(
                            spectral_model.metadata["fitted_result"][-1]
                            ["abundances"][0])
                    except:
    ...
    def optimize_stellar_parameters(self, **kwargs):
        ...
        initial_guess = [
            ...
            logg,
            MH,
        ]  # stupid me did not change the ordering to match
        logger.info(
            "Initializing optimization at Teff={:.0f} logg={:.2f} vt={:.2f} MH={:.2f}"
            .format(Teff, logg, vt, MH))

        # get the list of relevant spectral models.
        stellar_parameter_spectral_models = []
        for spectral_model in self.spectral_models:
            if (spectral_model.use_for_stellar_parameter_inference
                    and spectral_model.is_acceptable):
                if isinstance(spectral_model, SpectralSynthesisModel):
                    raise NotImplementedError(
                        "Syntheses cannot be used for stellar parameter state (check the Transitions manager)"
                    )
                stellar_parameter_spectral_models.append(spectral_model)
        transitions = self._spectral_models_to_transitions(
            stellar_parameter_spectral_models)
        finite = np.logical_and(
            np.isfinite(transitions["equivalent_width"]),
            transitions["equivalent_width"] > 0.01,
        )
        if finite.sum() != len(finite):
            logger.warn(
                "Number of finite transitions ({}) != number of transitions ({})"
                .format(finite.sum(), len(finite)))
        transitions = transitions[finite]
        logger.info("Optimizing with {} transitions".format(len(transitions)))

        # interpolator, do obj. function
        out = run_optimize_stellar_parameters(initial_guess, transitions,
                                              **kwargs)
        ...
        if not out[0]:
            logger.warn(
                "Optimization did not converge, stopped at Teff={:.0f} logg={:.2f} vt={:.2f} MH={:.2f}"
                .format(new_Teff, new_logg, new_vt, new_MH))
    ...
    def optimize_feh(self, params_to_optimize, **kwargs):
        ...
        initial_guess = np.array(
            [Teff, vt, logg,
             MH])  # stupid me did not change the ordering to match
        logger.info(
            "Initializing optimization at Teff={:.0f} logg={:.2f} vt={:.2f} MH={:.2f}"
            .format(Teff, logg, vt, MH))
        ...
        # interpolator, do obj. function
        out = run_optimize_feh(initial_guess, transitions, params_to_optimize,
                               **kwargs)
        final_parameters = out[1]
        new_Teff, new_vt, new_logg, new_MH = final_parameters
    ...
    def measure_abundances(self,
        ...
        for i, spectral_model in enumerate(spectral_models):
            if isinstance(spectral_model, ProfileFittingModel):
                ...
                if spectral_model.is_acceptable:
                    equivalent_widths.append(
                        1000.0 * spectral_model.metadata["fitted_result"][-1]
                        ["equivalent_width"][0])
                    equivalent_width_errs.append(
                        1000.0 *
                        np.nanmax(spectral_model.metadata["fitted_result"][-1]
                                  ["equivalent_width"][1:3]))
                else:
                    equivalent_widths.append(np.nan)
                    equivalent_width_errs.append(np.nan)
                num_profile += 1
        ...
        if num_profile > 0 and (len(equivalent_widths) == 0
        ...
        if save_abundances:
            for index, abundance, uncertainty in zip(
                    spectral_model_indices[finite], abundances, uncertainties):
                spectral_models[index].metadata["fitted_result"][-1][
                    "abundances"] = [abundance]
                spectral_models[index].metadata["fitted_result"][-1][
                    "abundance_uncertainties"] = [uncertainty]
        logger.info("Time to measure {} abundances: {:.1f}".format(
            np.sum(finite),
            time.time() - start))
        return abundances, uncertainties if calculate_uncertainties else abundances

    def summarize_spectral_models(
        self,
        spectral_models=None,
        organize_by_element=False,
        use_weights=None,
        use_finite=True,
        what_fe=1,
        default_error=0.1,
        ...
        def update_one_weight(key, err):
            if key not in all_weights:
                all_weights[key] = []
            all_weights[key].append(err**-2.0)
            return
        ...
        for spectral_model in spectral_models:
            ...
            for elem, species, logeps, logepserr in zip(
                    spectral_model.elements, spectral_model.species,
                    abundances, errors):
                ...
                if isinstance(key, list):
                    ...
                    for species in key:
                        if isinstance(species, float):
                            ...
                            for _species, _logeps, _logepserr in zip(
                                    species, logeps, logepserr):
                                update_one_key(_species, _logeps)
                                update_one_weight(_species, _logepserr)
                        else:
                            raise TypeError(
                                "key {} is of type {} (organized by {})".
                                format(species, type(species), what_key_type))
                    ...
                    raise TypeError(
                        "key {} is of type {} (organized by {})".format(
        ...
        for key in all_logeps:
            ...
            if use_weights:
                ...
                stdev = np.sqrt(
                    np.sum(weights * (logepss - logeps)**2) / np.sum(weights))
        ...
        total_num_models_summarized = np.sum(
            [len(x) for x in all_logeps.values()])
        return summary_dict
    ...
    def _export_ascii_abundance_table(self, filepath, summary_dict):
        ...
        for i, (species, (N, logeps, stdev, stderr, XH,
                          XFe)) in enumerate(iteritems(summary_dict)):
    ...
    def export_spectral_model_measurements(self, filepath):
        ...
        for i, spectral_model in enumerate(spectral_models):
            ...
            if isinstance(spectral_model, SpectralSynthesisModel):
                assert len(
                    spectral_model.elements) == 1, spectral_model.elements
                ...
                try:
                    logeps_err = (
                        spectral_model.metadata["2_sigma_abundance_error"] /
                        2.0)
                except:
                    logeps_err = np.nan
                ...
                try:
                    EW = (1000.0 * spectral_model.metadata["fitted_result"][2]
                          ["equivalent_width"][0])
                    # Erika added EW sigma to output
                    e_EW = max(1000.0 *
                               np.abs(spectral_model.metadata["fitted_result"]
                                      [2]["equivalent_width"][1:]))
                    logeps = spectral_model.abundances[0]
    ...
    def make_summary_plot(self, figure=None):
        ...
        if "summary_figure" not in defaults:
            raise RuntimeError(
                "Defaults file ({}) must have summary_figure".format(
    ...
    def copy_file_to_working_directory(self, filename, twd=None):
        ...
        while os.path.exists(path_to_copy):
            new_basename = ".".join([utils.random_string(), basename])
    ...
    def import_linelist_as_profile_models(self,
                                          filename,
                                          import_equivalent_widths=False,
        ...
        logger.debug("Time to load linelist {:.1f}".format(time.time() -
                                                           start))
        ...
        if import_equivalent_widths:
            try:
                ...
                raise KeyError(
                    "no equivalent widths found in imported line list")
        ...
        for i in range(len(line_list)):
            ...
            if import_equivalent_widths and np.isfinite(
                ...
                model.metadata.update({
                    ...
                    "fitted_result": [
                        ...
                        None,
                        {
                            ...
                            "equivalent_width": (
                                1e-3 * line["equivalent_width"],
                                0.0,
                                0.0,
                            ),
                            "reduced_equivalent_width": (
                                -3 + np.log10(line["equivalent_width"] /
                                              line["wavelength"]),
                                0.0,
                                0.0,
                            ),
        ...
        logger.debug("Created {} profile models in {:.1f}s".format(
            len(line_list),
            time.time() - start))
    ...
    def import_linelist_as_synthesis_model(self,
                                           filename,
                                           elements,
                                           copy_to_working_dir=False,
                                           **kwargs):
        ...
        start = time.time()
        spectral_model = SpectralSynthesisModel(self, line_list, elements,
                                                **kwargs)
        self.metadata["spectral_models"].append(spectral_model)
        logger.debug("Created synthesis model with {} lines in {:.1f}s".format(
            len(line_list),
            time.time() - start))
    ...
    def import_master_list(self,
        ...
        types = np.array(
            list(map(lambda x: x.lower(), np.array(master_list["type"]))))
        print(types)
        assert np.all([(x == "eqw") or (x == "syn") or (x == "list")
        ...
        for row in syn:
            elem1, elem2, isotope1, isotope2, ion = utils.species_to_elems_isotopes_ion(
                row["species"])
            logger.debug("{} -> {} {} {} {} {}".format(row["species"], elem1,
                                                       elem2, isotope1,
                                                       isotope2, ion))
            if elem2 == "":
                element = [elem1]
            ...
            kwargs = {
                "what_wavelength": what_wavelength,
                "what_species": what_species,
                "what_expot": what_expot,
                "what_loggf": what_loggf,
            }
            try:
                self.import_linelist_as_synthesis_model(
                    _filename,
                    element,
        ...
        if num_added != len(master_list):
            logger.warn(
                "Created {} models out of {} in the master list".format(
    ...
    def export_stitched_continuum(self, path):
        ...
        assert len(self.input_spectra) == len(
            self.metadata["normalization"]["continuum"])
        continuums = []
        for i, (spectrum, continuum) in enumerate(
                zip(self.input_spectra,
                    self.metadata["normalization"]["continuum"])):
            continuums.append(
                specutils.Spectrum1D(spectrum.dispersion, continuum,
                                     1.0 / continuum))
    ...
    def get_spectral_models_species_dict(self):
        ...
        for model in self.spectral_models:
            if isinstance(model, ProfileFittingModel):
                species = round(model.species[0], 1)
            if isinstance(model, SpectralSynthesisModel):
                if len(model.species) > 1:
                    warnings.warn(
                        "Spectral model has multiple species: {}".format(
                            model.species))
