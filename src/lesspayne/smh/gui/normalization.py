...
from lesspayne.smh.gui import mpl

# This is a bad idea, but it's April 1st.
from lesspayne.smh.gui.style_utils import wavelength_to_hex
...
class NormalizationTab(QtGui.QWidget):
    ...
    def __init__(self, parent):
        ...
        self.knot_spacing_label.setText("Knot spacing (Å)")
        ...
        self.blue_trim_label.setText("Blue trim (pixels)")
        ...
        self.red_trim_label.setText("Red trim (pixels)")
        ...
        settings_layout.addItem(
            QtGui.QSpacerItem(40, 20, QtGui.QSizePolicy.Minimum,
        ...
        # Line for the data.
        self.ax_order.plot([np.nan], [np.nan], c="k",
                           zorder=3)  # , drawstyle='steps-mid')
        ...
        self.ax_order.plot([np.nan], [np.nan],
                           c="#666666",
                           zorder=1,
                           drawstyle="steps-mid")
        ...
        self.ax_order_norm.set_xlabel("Wavelength (Å)")
        ...
        self.norm_plot.mpl_connect("key_press_event", self.figure_key_press)
        self.norm_plot.mpl_connect("button_press_event",
                                   self.figure_mouse_press)
        self.norm_plot.mpl_connect("button_release_event",
        ...
        self.order.currentIndexChanged.connect(self.update_normalization_order)
        self.norm_max_iter.currentIndexChanged.connect(
            self.update_normalization_max_iterations)
        self.low_sigma_clip.textChanged.connect(self.update_low_sigma_clip)
        self.high_sigma_clip.textChanged.connect(self.update_high_sigma_clip)
    ...
    def normalize_and_stitch(self):
        ...
        self.parent.tabs.setTabEnabled(
            self.parent.tabs.indexOf(self) + 1, True)
        self.parent.tabs.setTabEnabled(
            self.parent.tabs.indexOf(self) + 2, True)
        self.parent.tabs.setTabEnabled(
            self.parent.tabs.indexOf(self) + 3, True)
    ...
    def figure_key_press(self, event):
        ...
        if event.key in "aA":
            ...
            self._cache["input"]["additional_points"] = np.hstack(
                (points, 100 * np.ones(N).reshape((N, 1))))
    ...
    def figure_mouse_press(self, event):
        ...
        if event.dblclick:
            logger.info(
                "Removed double-click to add point. Use 'a' key instead")
            if event.button == 1:
                # Add a point.
                ...
                distance = np.sqrt(((points[:, 0] - event.xdata) / xscale)**2 +
                ...
                if distance.size > 0:
                    ...
                    if distance[index] < PIXEL_PICKER_TOLERANCE:
                        ...
                        print("Closest point {} px away".format(
                            distance[index]))
        ...
        try:
            ...
            self._exclude_selected_region.set_xy([[xmin, ymin], [xmin, ymax],
                                                  [xmax, ymax], [xmax, ymin],
                                                  [xmin, ymin]])
        ...
        self._exclude_selected_region_signal = (
            time(),
            self.norm_plot.mpl_connect("motion_notify_event",
                                       self.update_exclude_selected_region),
        )
        return None
    ...
    def _populate_widgets(self):
        ...
        self._cache = {"input": {}}
        for key in keys:
            self._cache["input"][key] = self.parent.session.setting(
                ("normalization", key))

        # Continuum masks.
        self._cache["masks"] = self.parent.session.setting(
            ("normalization", "masks"))
        self._cache["default_mask"] = (self.parent.session.setting(
            ("normalization", "default_mask"))
                                       or self._cache["masks"].keys()[0])

        # Put these values into the widgets.
        self.low_sigma_clip.setText(str(
            self._cache["input"]["low_sigma_clip"]))
        self.high_sigma_clip.setText(
            str(self._cache["input"]["high_sigma_clip"]))
        ...
        functions = [
            self.function.itemText(i).lower()
            for i in range(self.function.count())
        ]
        self.function.setCurrentIndex(
            functions.index(self._cache["input"]["function"]))

        # Normalization order.
        orders = [
            int(self.order.itemText(i)) for i in range(self.order.count())
        ]
        self.order.setCurrentIndex(orders.index(self._cache["input"]["order"]))

        # Normalization maximum iterations.
        norm_max_iters = [
            int(self.norm_max_iter.itemText(i))
            for i in range(self.norm_max_iter.count())
        ]
        self.norm_max_iter.setCurrentIndex(
            norm_max_iters.index(self._cache["input"]["max_iterations"]))
    ...
    def update_continuum_mask(self, refresh=False):
        ...
        try:
            vhelio = self.parent.session.metadata["rv"][
                "heliocentric_correction"]
            bcv_shift = self.parent.session.metadata["rv"][
                "barycentric_correction"]
            dop_shift = vhelio + bcv_shift
        except (AttributeError, KeyError):
            dop_shift = 0.0
        # -----------------------------------------------------------------
        _ = self.parent.session.metadata["normalization"][
        ...
        for start, end in masked_regions:
            ...
            polygons = (self._masked_wavelengths[i],
    ...
    def update_blue_trim(self):
        ...
        try:
            ...
            self._cache["input"]["exclude"] = np.array([[x[0],
                                                         x[trim_region]]])
            exclude = self._cache["input"]["exclude"]
        ...
        if len(exclude) == 0:
            ...
            self._cache["input"]["exclude"] = np.delete(exclude,
                                                        mask_to_delete,
                                                        axis=0)
            self._cache["input"]["exclude"] = np.insert(
                self._cache["input"]["exclude"],
                0, [[x[0], x[trim_region]]],
                axis=0)
    ...
    def update_red_trim(self):
        ...
        if len(exclude) == 0:
            ...
            self._cache["input"]["exclude"] = np.delete(exclude,
                                                        mask_to_delete,
                                                        axis=0)
            self._cache["input"]["exclude"] = np.append(
                self._cache["input"]["exclude"],
                [[x[-trim_region], x[-1] + 1e-3]],
                axis=0,
            )
    ...
    def update_normalization_max_iterations(self):
        ...
        self._cache["input"]["max_iterations"] = int(
            self.norm_max_iter.currentText())
        self.fit_continuum(True)
        self.draw_continuum(True)
        self.reset_input_style_defaults()
        return None
    ...
    def update_order_index(self, index=None):
        ...
        self.current_order = session.input_spectra[
            self.current_order_index].copy()
    ...
    def update_order_figure(self, index=None):
        ...
        self.current_order_label.setText("Order {} of {}".format(
            1 + self.order_slide.value(),
            len(self.parent.session.input_spectra)))

        # Do all of the orders have continuum? If so, update the button.
        if not any([(_ is None)
                    for _ in self.parent.session.metadata["normalization"]
                    ["continuum"]]):
    ...
    def check_for_different_input_settings(self):
        ...
        try:
            ...
            logger.debug(
                "check_for_different_input_settings: no continuum kwd")
        normalization_kwargs = session.metadata["normalization"][
            "normalization_kwargs"][index]
        ...
        self.high_sigma_clip.setText(
            str(self._cache["input"]["high_sigma_clip"]))
        self.knot_spacing.setText(
            str(int(self._cache["input"]["knot_spacing"])))
        self.blue_trim.setText(str(self._cache["input"]["blue_trim"]))
        self.red_trim.setText(str(self._cache["input"]["red_trim"]))
        ...
        self.function.setCurrentIndex(
            functions.index(self._cache["input"]["function"].lower()))
        self.order.setCurrentIndex(orders.index(self._cache["input"]["order"]))
        self.norm_max_iter.setCurrentIndex(
            norm_max_iters.index(self._cache["input"]["max_iterations"]))
    ...
    def reset_input_style_defaults(self, items=None):
        ...
        items = items or (
            ...
            self.max_iter_label,
            self.norm_max_iter,
            self.low_sigma_clip_label,
            self.low_sigma_clip,
            self.high_sigma_clip_label,
            self.high_sigma_clip,
            self.knot_spacing_label,
            self.knot_spacing,
            self.blue_trim_label,
            self.blue_trim,
            self.red_trim_label,
            self.red_trim,
        )
        ...
        for item in items:
            item.setStyleSheet("{0} {{ font-weight: normal }}".format(
                item.__class__.__name__))
            item.setStatusTip("")
        self.parent.statusbar.showMessage(
            self.parent._default_statusbar_message)
        return None
    ...
    def draw_order(self, refresh=False):
        ...
        percent = 2
        trimming = (x[-1] - x[0]) * percent / 100.0
        self.ax_order.set_xlim(x[0] - trimming, x[-1] + trimming)

        trimming = (np.nanmax(y) - np.nanmin(y)) * percent / 100.0
        self.ax_order.set_ylim(
            np.nanmin(y) - trimming,
            np.nanmax(y) + trimming)
        ...
        try:
            ...
            if sender is not None:
                ...
                sender.setStyleSheet(
                    "{0} {{ background-color: #f6989d; }}".format(
    ...
    def draw_continuum(self, refresh=False):
        ...
        try:
            knots = self.current_order.get_knots(kwds["knot_spacing"],
                                                 kwds["exclude"])
            knoty = continuum[np.searchsorted(self.current_order.dispersion,
                                              knots)]
