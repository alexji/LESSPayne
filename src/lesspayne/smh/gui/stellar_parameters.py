...
from lesspayne.smh.gui import style_utils, mpl
...
from lesspayne.smh.gui.base import (
    ...
    MeasurementTableDelegate,
)
from lesspayne.smh.gui.base import MeasurementTableModelBase, MeasurementTableModelProxy
from lesspayne.smh.gui.base import SMHScatterplot
from lesspayne.smh.photospheres import available as available_photospheres
...
from lesspayne.smh.optimize_stellar_params import (
    optimize_stellar_parameters,
    optimize_feh,
)
...
class StellarParametersTab(QtGui.QWidget):
    ...
    def __init__(self, parent):
        ...
        self.params_to_optimize = np.array([True] *
                                           4)  # the params to optimize
        self.optimize_vt_to_fe2 = (
            False  # whether to optimize vt to [Fe I/H] or [Fe II/H]
        )
    ...
    def measure_abundances(self):
        ...
        for spectral_model in self.parent.session.spectral_models:
            if (isinstance(spectral_model, ProfileFittingModel)
                    ...
                    and spectral_model.use_for_stellar_parameter_inference
                    and (not spectral_model.is_upper_limit)):
    ...
    def update_stellar_parameter_state_table(self):
        ...
        try:
            ...
            logger.debug(e)
            mchi1, bchi1, med1, eXH1, emchi1, N1 = (
                np.nan,
                np.nan,
                np.nan,
                np.nan,
        ...
        try:
            ...
            logger.debug(e)
            mREW1, bREW1, med1, eXH1, emREW1, N1 = (
                np.nan,
                np.nan,
                np.nan,
                np.nan,
        ...
        try:
            ...
            logger.debug(e)
            mchi2, bchi2, med2, eXH2, emchi2, N2 = (
                np.nan,
                np.nan,
                np.nan,
                np.nan,
        ...
        try:
            ...
            logger.debug(e)
            mREW2, bREW2, med2, eXH2, emREW2, N2 = (
                np.nan,
                np.nan,
                np.nan,
                np.nan,
        ...
        self.state_fe1_N.setText("Fe I ({})".format(N1))
        self.state_fe1_XH.setText("{:.2f} ± {:.2f}".format(XH1, eXH1))
        self.state_fe1_dAdchi.setText("{:.3f} ± {:.3f}".format(mchi1, emchi1))
        self.state_fe1_dAdREW.setText("{:.3f} ± {:.3f}".format(mREW1, emREW1))
        self.state_fe2_N.setText("Fe II ({})".format(N2))
        self.state_fe2_XH.setText("{:.2f} ± {:.2f}".format(XH2, eXH2))
        self.state_fe2_dAdchi.setText("{:.3f} ± {:.3f}".format(mchi2, emchi2))
        self.state_fe2_dAdREW.setText("{:.3f} ± {:.3f}".format(mREW2, emREW2))
        return None
    ...
    def solve_feh(self):
        ...
        logger.info("Setting [alpha/Fe]=0.4 to solve")
        self.update_stellar_parameter_session()
        self.parent.session.optimize_feh(
            self.params_to_optimize,
            optimize_vt_to_fe2=self.optimize_vt_to_fe2)
        self.parent.session.metadata["stellar_parameters"]

        ## refresh everything
        # TODO: fix this state update via a better force bc delay is long
        self.new_session_loaded()
        self.measure_abundances()
        self.new_session_loaded()
    ...
    def _init_rt_options(self, parent):
        ...
        # on change, set session optimizer to use this schema
        self.vt_opt_cb.currentIndexChanged.connect(
            self.set_session_vt_optimize)
    ...
    def _init_rt_buttons(self, parent):
        ...
        # WARNING: BRO WHAT DO THESE DO. WHY ARE THERE 3 OPTIMIZER FUNCTIONS.
        # self.btn_solve_feh = QtGui.QPushButton(self)
        # self.btn_solve_feh.setText("Solve only [M/H] and vt")
    ...
    def _init_state_table(self, parent):
        ...
        create_label("〈[X/H]〉", 0, 1)
        create_label("∂A/∂χ", 0, 2)
        create_label("∂A/∂REW", 0, 3)

        # Create State Labels
        self.state_fe1_N = create_label("Fe I (nan)",
                                        1,
                                        0,
                                        align=QtCore.Qt.AlignLeft)
        self.state_fe1_XH = create_label("nan ± nan", 1, 1)
        self.state_fe1_dAdchi = create_label("nan ± nan", 1, 2)
        self.state_fe1_dAdREW = create_label("nan ± nan", 1, 3)

        self.state_fe2_N = create_label("Fe II (nan)",
                                        2,
                                        0,
                                        align=QtCore.Qt.AlignLeft)
        self.state_fe2_XH = create_label("nan ± nan", 2, 1)
        self.state_fe2_dAdchi = create_label("nan ± nan", 2, 2)
        self.state_fe2_dAdREW = create_label("nan ± nan", 2, 3)
    ...
    def _check_for_spectral_models(self):
        ...
        for sm in self.parent.session.metadata.get("spectral_models", []):
            if sm.use_for_stellar_parameter_inference:
                break
        else:
...
class StellarParameterUncertaintiesDialog(QtGui.QDialog):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i, label in enumerate([
                self.label_staterr_Teff,
                self.label_staterr_logg,
                self.label_staterr_MH,
                self.label_staterr_vt,
        ]):
            label.setSizePolicy(
                QtGui.QSizePolicy(QtGui.QSizePolicy.MinimumExpanding,
                                  QtGui.QSizePolicy.Minimum))
        ...
        for i, edit in enumerate([
                self.edit_syserr_Teff,
                self.edit_syserr_logg,
                self.edit_syserr_MH,
                self.edit_syserr_vt,
        ]):
            edit.setMinimumSize(QtCore.QSize(40, 0))
        ...
        for i, label in enumerate([
                self.label_toterr_Teff,
                self.label_toterr_logg,
                self.label_toterr_MH,
                self.label_toterr_vt,
        ]):
            label.setSizePolicy(
                QtGui.QSizePolicy(QtGui.QSizePolicy.MinimumExpanding,
                                  QtGui.QSizePolicy.Minimum))
    ...
    def calc_stat_err(self):
        ...
        staterr_Teff, staterr_logg, staterr_vt, staterr_MH = (
            self.session.stellar_parameter_uncertainty_analysis(
    ...
    def save_to_session(self):
        self.session.set_stellar_parameters_errors(
            "stat",
