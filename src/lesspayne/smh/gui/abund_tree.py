...
def summarize_abundances_species(ttab, use_weights=True):
    ...
    N = len(ttab)
    if N == 0:
        return [element, N, np.nan, np.nan, np.nan, np.nan]
    if use_weights:
        weights = 1.0 / ttab["e(X)"]**2
    else:
        weights = np.ones(N)
...
class AbundTreeElementSummaryItem(AbundTreeItem):
    ...
    def _getChildren(self):
        ...
        return [
            AbundTreeMeasurementItem(row, self.sm_indices[row],
                                     self.ab_indices[row], self)
            for row in range(N)
        ]
    ...
    def print_summary(self):
        return "{0:5} N={1:3} A(X)={2:5.2f} e(X)={3:5.2f} [X/H]={4:5.2f} [X/Fe]={5:5.2f}".format(
            *self.summary)
    ...
    def data(self, column):
        ...
        if isinstance(m, ProfileFittingModel):
            if column == 1:  # wl
                ...
                try:
                    return "{:5.2f}".format(
                        m.metadata["fitted_result"][2]["abundances"][0])
                except:
                    return str(np.nan)
            elif column == 3:  # equivalent_width
                try:
                    return "{:6.2f}".format(
                        m.metadata["fitted_result"][2]["equivalent_width"][0] *
                        1000.0)
                except:
                    return str(np.nan)
            elif column == 4:  # REW
                try:
                    return "{:6.2f}".format(
                        np.log10(m.metadata["fitted_result"][2]
                                 ["equivalent_width"][0] /
                                 m.transitions["wavelength"][0]))
                except:
                    return str(np.nan)
            elif column == 5:  # e(X)
                # TODO
...
class AbundTreeModel(QtCore.QAbstractItemModel):
    ...
    def obtain_measurements_from_parent_tab(self):
        logger.debug("Sorting measurements...")
        start = time.time()
    ...
    def _getSummaries(self):
        ...
        for index, species in enumerate(self.all_species):
            ...
            elem_summary = AbundTreeElementSummaryItem(None, sm_indices,
                                                       ab_indices, index, self)
    ...
    def setData(self, index, value, role):
        ...
        if isinstance(item, AbundTreeMeasurementItem) and index.column() == 0:
            ...
            if role == QtCore.Qt.CheckStateRole:
                ...
                botRight = self.createIndex(0, item.parent.columnCount(),
                                            item.parent)
...
if __name__ == "__main__":
    ...
    print("Loading pre-saved spectral models")
    start = time.time()
