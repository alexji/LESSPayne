...
from .base import BaseTableView, MeasurementTableModelBase
...
class TransitionsDialog(QtGui.QDialog):
    ...
    def _create_table(self):
        tablemodel = MeasurementTableModelBase(
            self, self.session, MeasurementTableModelBase.allattrs)
    ...
    def _create_buttons(self):
        ...
        self.btn_import_spec_models.clicked.connect(
            self.import_spectral_model_states)
        ...
        self.btn_export_spec_models.clicked.connect(
            self.export_spectral_model_states)
    ...
    def add_profile_list(self):
        if self.session is None:
            return None
        paths = self.open_file(caption="Select linelists for profiles",
                               dir="",
                               filter="")
        if not paths:
            return None
    ...
    def add_synth_list(self):
        ...
        for path in paths:
            ...
            selectable_elements = list(
                set(transitions.unique_elements).difference(["H"]))
            ...
            self.session.import_linelist_as_synthesis_model(
                path, dialog.selected_elements)
    ...
    def import_spectral_model_states(self):
        if self.session is None:
            return None
        paths = self.open_file(caption="Select spectral model state file",
                               dir="",
                               filter="*.pkl")
        self.tableview.model().beginResetModel()
        for path in paths:
    ...
    def import_master_list(self):
        if self.session is None:
            return None
        paths = self.open_file(caption="Select master lists",
                               dir="",
                               filter="")
        self.tableview.model().beginResetModel()
        for path in paths:
    ...
    def save_as_default(self):
        ...
        self.session.metadata["line_list"].write(path,
                                                 format="fits",
                                                 overwrite=True)
        ...
        self.session.update_default_setting(("default_spectral_models", ),
                                            states)
...
class LineListTableView(QtGui.QTableView):
    ...
    def add_imported_lines_as_synthesis_model(self,
        ...
        selected = self.import_from_filename(filenames=filenames,
                                             full_output=True)
        if selected is None:
            return None
        ...
        self._parent.models_view.model().beginResetModel()
        if input_elements is not None and len(filenames) == len(
                input_elements):
            for elements_to_measure in input_elements:
                assert len(elements_to_measure) >= 1
                # TODO verify they are elements in the list
            for filename, transitions, elements_to_measure in zip(
                    filenames, filename_transitions, input_elements):
            ...
            for filename, transitions in zip(filenames, filename_transitions):
                if len(transitions.unique_elements) == 1:
                    ...
                    dialog = PeriodicTableDialog(
                        selectable_elements=selectable_elements,
                        explanation=
                        "Please select which element(s) will be measured"
                        " by synthesizing the transitions in {}:".format(
                    ...
                    self.session.metadata["spectral_models"].append(
                        SpectralSynthesisModel(self.session,
                                               transitions["hash"],
    ...
    def delete_selected_rows(self):
        ...
        self._parent.models_view.model().beginResetModel()
        self.session.metadata["line_list"] = self.session.metadata[
            "line_list"][mask]
    ...
    def import_from_filename(self, filenames=None, full_output=False):
        ...
        ta = time()
        self.model().beginResetModel()
        out = self.session.import_linelists(
            filenames,
            ignore_conflicts=self._parent.checkbox_merge_without_conflicts.
            isChecked(),
    ...
    def import_transitions_with_measured_equivalent_widths(
            self, filenames=None):
        """Import profile models with pre-measured equivalent widths."""

        if filenames is None:
            filenames, selected_filter = QtGui.QFileDialog.getOpenFileNames(
                self, caption="Select pre-measured transition files", dir="")
        else:
            if isinstance(filenames, string_types):
                filenames = [filenames]
        if not filenames:
...
class SpectralModelsTableModel(QtCore.QAbstractTableModel):
    ...
    def data(self, index, role):
        if (role == QtCore.Qt.CheckStateRole and index.isValid()
            ...
            return QtCore.Qt.Checked if getattr(sm,
                                                attr) else QtCore.Qt.Unchecked
...
class SpectralModelsTableView(QtGui.QTableView):
    ...
    def contextMenuEvent(self, event):
        ...
        if not any_selected:
            ...
            values = list(
                set(self.session.metadata["spectral_models"][
                    row.row()].metadata[a] for row in selected))
    ...
    def delete_selected_rows(self):
        ...
        delete_indices = [
            _.row() for _ in self.selectionModel().selectedRows()
        ]

        self.model().beginResetModel()
        self.session.metadata["spectral_models"] = [
            sm for i, sm in enumerate(self.session.metadata["spectral_models"])
    ...
    def export_selected_spectral_models(self):
        ...
        path, _ = QtGui.QFileDialog.getSaveFileName(
            self,
            caption="Export spectral models to disk",
            dir="",
            filter="*.pkl")
        if not path:
            return
        ...
        line_list_subset = self.session.metadata["line_list"][
            transition_indices]
