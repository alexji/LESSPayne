...
class RVTab(QtGui.QWidget):
    ...
    def __init__(self, parent):
        ...
        self.ax_order_norm.plot([np.nan], [np.nan],
                                c="k",
                                drawstyle="steps-mid")
    ...
    def _populate_widgets(self):
        ...
        self._cache = {"input": rv_dict.copy()}

        # Wavelength regions should just be a single range.
        try:
            self._cache["input"]["wavelength_region"] = self._cache["input"][
                "wavelength_regions"][0]
        except KeyError:
            self._cache["input"][
                "wavelength_regions"] = self.parent.session.setting(
                    ("rv", "wavelength_regions"))
            rv_dict["wavelength_regions"] = self.parent.session.setting(
                ("rv", "wavelength_regions"))
            self._cache["input"]["wavelength_region"] = self._cache["input"][
                "wavelength_regions"][0]
        ...
        if isinstance(rv_dict.get("template_spectrum", ""),
                      string_types) and not os.path.exists(
                          rv_dict.get("template_spectrum", "")):
        ...
        if isinstance(rv_dict.get("template_spectrum", ""), string_types):
            ...
            self._cache["input"]["template_spectrum_path"] = rv_dict[
                "template_spectrum"]
        else:
        ...
        for each in rv_dict["wavelength_regions"]:
            self.wl_region.addItem("{0:.0f}-{1:.0f} Å".format(*each))

        # Normalization function.
        norm_functions = [
            self.norm_function.itemText(i).lower()
            for i in range(self.norm_function.count())
        ]
        self.norm_function.setCurrentIndex(
            norm_functions.index(rv_dict["normalization"]["function"].lower()))

        # Normalization order.
        norm_orders = [
            int(self.norm_order.itemText(i))
            for i in range(self.norm_order.count())
        ]
        self.norm_order.setCurrentIndex(
            norm_orders.index(rv_dict["normalization"]["order"]))
        ...
        self.norm_max_iter.setCurrentIndex(
            norm_max_iters.index(rv_dict["normalization"]["max_iterations"]))
    ...
    def update_normalization_order(self):
        ...
        self._cache["input"]["normalization"]["order"] = int(
            self.norm_order.currentText())
        self.fit_and_redraw_normalized_order()

    def update_normalization_max_iterations(self):
        """Update the maximum number of iterations during normalization."""
        self._cache["input"]["normalization"]["max_iterations"] = int(
    ...
    def update_normalization_low_sigma(self):
        ...
        if low_sigma:
            self._cache["input"]["normalization"]["low_sigma_clip"] = float(
                low_sigma)
            self.fit_and_redraw_normalized_order()
    ...
    def update_normalization_high_sigma(self):
        ...
        if high_sigma:
            self._cache["input"]["normalization"]["high_sigma_clip"] = float(
                high_sigma)
            self.fit_and_redraw_normalized_order()
    ...
    def update_normalization_knot_spacing(self):
        ...
        if knot_spacing:
            self._cache["input"]["normalization"]["knot_spacing"] = float(
                knot_spacing)
            self.fit_and_redraw_normalized_order()
    ...
    def fit_continuum(self):
        ...
        self._cache["normalized_order"], self._cache[
            "continuum"], _, __ = self._cache["overlap_order"].fit_continuum(
                full_output=True, **self._cache["input"]["normalization"])
        return None
    ...
    def redraw_continuum(self, refresh=False):
        ...
        self.ax_order.lines[1].set_data([
            self._cache["overlap_order"].dispersion, self._cache["continuum"]
        ])
        if refresh:
            self.rv_plot.draw()
        return None
    ...
    def cross_correlate(self):
        ...
        if isinstance(kwds.get("template_spectrum", ""),
                      string_types) and not os.path.exists(
                          kwds.get("template_spectrum", "")):
            ...
            kwds["template_spectrum"] = self._cache["input"][
                "template_spectrum"]
    ...
    def update_wl_region(self, verbose=True):
        ...
        if current_text == "":
            return
        wavelength_region = [
            float(_) for _ in current_text.split(" ")[0].split("-")
        ]
        ...
        self.ax_order.set_ylim(flux_limits[0],
                               flux_limits[1] + (np.ptp(flux_limits) * 0.10))

        # TODO: This may require some updating.
        print(
            "Assuming that the session has not just been loaded and it has a CCF/norm order, etc"
        )
    ...
    def draw_template(self, refresh=False):
        ...
        self.ax_order_norm.lines[2].set_data(
            [template.dispersion, template.flux])
...
class RVRegionDialog(QtGui.QDialog):
    ...
    def __init__(self, rv_tab, *args):
        ...
        for each in wavelength_regions:
            self.listWidget.addItem("{0:.0f}-{1:.0f} Å".format(*each))
    ...
    def save_to_session(self):
        regions = self.get_regions_from_list()
        self.rv_tab.parent.session.metadata["rv"][
            "wavelength_regions"] = regions
        logger.debug("Saved wavelength regions to session: {}".format(regions))

    def save_to_list(self):
        wavelength_region = self.get_wavelength_region()
        if wavelength_region == None:
            return None
        self.listWidget.addItem("{0:.0f}-{1:.0f} Å".format(*wavelength_region))
    ...
    def get_wavelength_region(self):
        ...
        if wl_lower is None or wl_upper is None:
            return None
    ...
    def redraw_normalized_order(self, refresh=False):
        ...
        # Redshift the normalized order by the 'RV-applied', if it exists.
        try:
            rv_applied = self.rv_tab.parent.session.metadata["rv"][
                "rv_applied"]
        except (AttributeError, KeyError):
            rv_applied = 0

        self.ax_order_norm.lines[1].set_data([
            self.rv_tab._cache["normalized_order"].dispersion *
            (1 + rv_applied / c),
            self.rv_tab._cache["normalized_order"].flux,
        ])
        self.ax_order_norm.set_xlim(
            self.rv_tab._cache["input"]["wavelength_region"])
