...
from lesspayne.smh import utils, LineList
...
_labels = [
    ...
    "Upper Limit",
    "User Flag",
    "Use for Spectroscopic Stellar Parameters",
    "Use for Stellar Abundances",
    "Measurement Type",
    "FWHM",
]
_short_labels = [
    ...
    * Shift-rightclick to pan [TODO]
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(SMHSpecDisplay, self).__init__(parent=parent,
                                             session=session,
                                             **kwargs)
        self.parent = parent
        ...
        gs.update(top=0.95, bottom=0.05, hspace=0)
        ...
        if enable_masks:
            ...
            self.mpl_connect("button_press_event",
                             self.spectrum_left_mouse_press)
            self.mpl_connect("button_release_event",
                             self.spectrum_left_mouse_release)
        ...
        self._lines = {
            "comparison_spectrum":
            self.ax_spectrum.plot([np.nan], [np.nan],
                                  c="c",
                                  alpha=0.5,
            ...
            self.ax_spectrum.plot([np.nan], [np.nan],
                                  c="k",
                                  drawstyle="steps-mid")[0],  # None,
            "spectrum_fill":
            None,
            "residual_fill":
            None,
            "transitions_center_main":
            self.ax_spectrum.axvline(np.nan, c="#666666", linestyle=":"),
            "transitions_center_residual":
            self.ax_residual.axvline(np.nan, c="#666666", linestyle=":"),
            "linelabels":
            self.ax_spectrum.vlines(np.nan, np.nan, np.nan, color="blue",
                                    lw=1),
            "weak_linelabels":
            self.ax_spectrum.vlines(np.nan,
                                    np.nan,
                                    np.nan,
                                    color="blue",
                                    linestyle=":",
            ...
            "nearby_lines": [],
            "model_fit":
            self.ax_spectrum.plot([np.nan], [np.nan], c="r")[0],
            "model_residual":
            self.ax_residual.plot([np.nan], [np.nan],
                                  c="k",
                                  drawstyle="steps-mid")[0],
            "interactive_mask": [
    ...
    def reset(self):
        ...
        if self.session is not None:
            drawstyle = self.session.setting(
                ["plot_styles", "spectrum_drawstyle"], "steps-mid")
    ...
    def update_spectrum_figure(self,
        ...
        if label_transitions is not None:
            logger.info("labelling {} transitions for {} (rv={})".format(
                len(label_transitions),
                np.array(np.unique(label_transitions["species"])),
                label_rv,
            ))
        if self.session is None:
            return None
    ...
    def label_lines(
        ...
        """
        if ymin is None:
            ymin = self.label_ymin
        if ymax is None:
            ymax = self.label_ymax
        ...
        ii_strong = np.logical_and(
            transitions["expot"] < strong_expot_max,
    ...
    def key_press_zoom(self, event):
        if event.key not in ["1", "2", "3", "4", "left", "right"]:
            return None
        if self.session is None:
            return None
        if event.key in "1234":
            ylim = self.session.setting(
                ["zoom_shortcuts", int(event.key)],
    ...
    def spectrum_left_mouse_press(self, event):
        ...
        if event.dblclick:
            ...
            if mask_removed and isinstance(selected_model,
                                           ProfileFittingModel):
                selected_model.fit()
        ...
        if selected_model.metadata["antimask_flag"] != self.shift_key_pressed:
            ...
            selected_model.metadata[
                "antimask_flag"] = not selected_model.metadata["antimask_flag"]
            logger.debug("Switching antimask flag to {}".format(
        ...
        for patch in self._lines["interactive_mask"]:
            patch.set_xy([[xmin, ymin], [xmin, ymax], [xmax, ymax],
                          [xmax, ymin], [xmin, ymin]])
            patch.set_facecolor(
                "g" if selected_model.metadata["antimask_flag"] else "r")

        # Set the signal and the time.
        self._interactive_mask_region_signal = (
            time.time(),
            self.mpl_connect("motion_notify_event", self.update_mask_region),
        )
    ...
    def spectrum_left_mouse_release(self, event):
        ...
        if (time.time() - signal_time > DOUBLE_CLICK_INTERVAL
            ...
            if spectral_model is None:
                raise RuntimeError(
                    """Must have a spectral model selected while making mask!
    ...
    def _plot_normalized_spectrum(self, limits, extra_disp=10):
        if self.session is None:
            return False
        if not hasattr(self.session, "normalized_spectrum"):
            return False
        ...
        plot_ii = np.logical_and(
            spectrum.dispersion > limits[0] - extra_disp,
        ...
        # Draw the spectrum.
        self._lines["spectrum"].set_data(spectrum.dispersion[plot_ii],
                                         spectrum.flux[plot_ii])

        # Draw the error bars.
        sigma = 1.0 / np.sqrt(spectrum.ivar[plot_ii])
        self._lines["spectrum_fill"] = style_utils.fill_between_steps(
            self.ax_spectrum,
            spectrum.dispersion[plot_ii],
            spectrum.flux[plot_ii] - sigma,
            spectrum.flux[plot_ii] + sigma,
            facecolor="#cccccc",
            edgecolor="#cccccc",
            alpha=1,
        ...
        # Draw the error bars.
        self._lines["residual_fill"] = style_utils.fill_between_steps(
            self.ax_residual,
            spectrum.dispersion[plot_ii],
            -sigma,
            +sigma,
    ...
    def _plot_masks(self):
        if self.session is None:
            return False
        selected_model = self.selected_model
        mask_color = ("g" if "antimask_flag" in selected_model.metadata
                      and selected_model.metadata["antimask_flag"] else "r")
    ...
    def _plot_model(self):
        ...
        try:
            ...
            self._lines["model_fit"].set_linestyle(
                "-" if self.selected_model.is_acceptable else "--")
            self._lines["model_fit"].set_color(
                "r" if self.selected_model.is_acceptable else "b")
            ...
            if np.any(np.isfinite(meta["model_yerr"])):
                self._lines["model_yerr"] = self.ax_spectrum.fill_between(
                    ...
                    meta["model_y"] - meta["model_yerr"],
                    facecolor="r"
                    if self.selected_model.is_acceptable else "b",
                    edgecolor="none",
                    alpha=0.5,
                )
...
class SMHScatterplot(mpl.MPLWidget):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if error_styles is not None:
            assert (e_xattr is not None) or (e_yattr is not None), (
                "Must specify e_xattr and/or e_yattr for error_styles")
            assert (exattr is None) or (exattr in self.allattrs), exattr
            assert (eyattr is None) or (eyattr in self.allattrs), eyattr
        self.exattr = exattr
        self.eyattr = eyattr

        super(SMHScatterplot, self).__init__(parent=parent, **kwargs)
        ...
        if error_styles is None:
            ...
            raise NotImplementedError(
                "Need to implement error bar graphics updating!")
        ...
        for filt, point_kw, error_kw, linefit_kw, linemean_kw in zip(
                filters, point_styles, error_styles, linefit_styles,
                linemean_styles):
            if point_kw is None:
                point_objs.append(None)
            else:
                point_objs.append(
                    self.ax.scatter([], [],
            ...
            if linefit_kw is None:
                linefit_objs.append(None)
            else:
                linefit_objs.append(
            ...
            if linemean_kw is None:
                ...
                linemean_objs.append(self.ax.axhline(np.nan, **linemean_kw))
    ...
    def reset(self):
        ...
        for filt, point, error, linefit, linemean in self._graphics:
            if point is not None:
                point.set_offsets(np.array([np.nan, np.nan]).T)
    ...
    def linkToTable(self, tableview):
        ...
        assert isinstance(
            tablemodel, MeasurementTableModelProxy) or isinstance(
                tablemodel, MeasurementTableModelBase), type(tablemodel)
        self.tableview = tableview
        self.tablemodel = tablemodel
        assert self.xattr in self.tablemodel.attrs, (self.xattr,
                                                     self.tablemodel.attrs)
        assert self.yattr in self.tablemodel.attrs, (self.yattr,
                                                     self.tablemodel.attrs)
        ...
        if self.exattr is None:
            ...
            assert self.exattr in self.tablemodel.attrs, (
                self.exattr,
                self.tablemodel.attrs,
            )
        ...
        if self.eyattr is None:
            ...
            assert self.eyattr in self.tablemodel.attrs, (
                self.eyattr,
                self.tablemodel.attrs,
            )
    ...
    def update_scatterplot(self, redraw=False):
        if self.tableview is None or self.tablemodel is None:
            return None
        xs, ys, exs, eys = [], [], [], []
        Nrows = self.tablemodel.rowCount()
        if Nrows == 0:
            return None
        spectral_models = self.tablemodel.get_models_from_rows(
            np.arange(Nrows))
        ...
        for ifilt, (filt, point, error, linefit,
                    linemean) in enumerate(self._graphics):
            valid = valids[ifilt, :]
            nonzero = valid.sum() > 0
            x, y = xs[valid], ys[valid]
            if point is not None:
                if nonzero:
                    point.set_offsets(np.array([x, y]).T)
                else:
                    point.set_offsets(np.array([np.nan, np.nan]).T)
            if error is not None:
    ...
    def update_selected_points(self, redraw=False):
        ...
        if self.do_not_select_unacceptable and len(rows) > 0:
            not_acceptable = np.logical_not(
                self.tablemodel.get_data_column("is_acceptable"))
...
class MeasurementTableView(BaseTableView):
    ...
    def contextMenuEvent(self, event):
        ...
        fit_selected_models = menu.addAction("Fit selected model{}..".format(
            ["", "s"][N != 1]))
        ...
        if self.display_fitting_options:
            ...
            set_continuum_order = [
                continuum_menu.addAction("Order {}".format(i), checkable=True)
                for i in range(0, 10)
            ]
            ...
            enable_central_weighting = menu.addAction(
                "Enable central weighting")
            disable_central_weighting = menu.addAction(
                "Disable central weighting")
        ...
        if self.display_fitting_options:
            ...
            elif action in (enable_central_weighting,
                            disable_central_weighting):
    ...
    def set_flag(self, flag_field, toggle):
        assert flag_field in ["is_acceptable", "is_upper_limit",
                              "user_flag"], (flag_field)
    ...
    def set_fitting_option_value(self,
                                 key,
                                 value,
        ...
        for row, spectral_model in zip(rows, spectral_models):
            ...
            if not spectral_model.is_acceptable:
                num_unacceptable += 1
                continue
            if valid_for_profile and isinstance(spectral_model,
                                                ProfileFittingModel):
                num_profile_models += 1
                spectral_model.metadata[key] = value
                run_fit = True
            if valid_for_synth and isinstance(spectral_model,
            ...
            if run_fit and "fitted_result" in spectral_model.metadata:
                ...
                try:
                    ...
                    num_error += 1
                    logger.exception(
                        "Error fitting row {} after modifying {} to {}".format(
                            row, key, value))
                self.update_row(row)
        logger.info(
            "Changed {0}={1}, fit {2} out of {3} models ({4} profile, {5} synth, {6} unacceptable, {7} fit fail)"
            .format(
                key,
                value,
                num_fit,
    ...
    def set_fitting_window(self):
        ...
        if not is_ok:
            return None
        self.set_fitting_option_value("window",
                                      window,
    ...
    def set_continuum_order(self, order):
        self.set_fitting_option_value("continuum_order",
                                      order,
                                      valid_for_profile=True,
                                      valid_for_synth=True)
        return None
    ...
    def set_profile(self, kind):
        self.set_fitting_option_value("profile",
                                      kind,
                                      valid_for_profile=True,
                                      valid_for_synth=False)
        return None
    ...
    def set_central_weighting(self, toggle):
        self.set_fitting_option_value("central_weighting",
                                      toggle,
                                      valid_for_profile=True,
                                      valid_for_synth=False)
        return None
...
def create_measurement_table_with_buttons(parent, filtermodel, session,
                                          **kwargs):
    """
    ...
    show_or_hide_unacceptable = lambda: filtermodel.show_or_hide_unacceptable(
        btn_filter)
...
class MeasurementTableDelegate(QtGui.QItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if option.state & QtGui.QStyle.State_Selected:
            painter.setBrush(
                QtGui2.QBrush(self.parent().palette().highlight().color()))
        else:
...
class MeasurementTableModelProxy(QtCore.QSortFilterProxyModel):
    ...
    def show_or_hide_unacceptable(self, btn):
        ...
        if hide:
            self.add_filter_function("is_acceptable",
    ...
    def mapToSource(self, proxy_index):
        ...
        try:
            ...
            print(
                "INDEX ERROR IN TABLE: probably you loaded a new file while 'hide unacceptable' was activated"
            )
            print(e)
...
class MeasurementTableModelBase(QtCore.QAbstractTableModel):
    ...
    def get_data_column(self, column, rows=None):
        ...
        data = np.array([np.ravel(getter(r)) for r in rows],
                        dtype=_attr2dtype[attr])
        return data
    ...
    def data(self, index, role):
        ...
        if isinstance(value, (list, np.ndarray)):
            try:
                if isinstance(
                        value[0],
                    (list, np.ndarray)):  # list of lists for syntheses species
                    mystrs = [[fmt.format(v) for v in vlist]
                              for vlist in value]
                    mystrs = [item for sublist in mystrs for item in sublist]
                else:
                    mystrs = [fmt.format(v) for v in value]
            except ValueError as e:
                logger.debug("A: {} has fmt {} and failing on {}".format(
                    attr, fmt, value))
                mystr = str(value)
            else:
                mystr = ";".join(mystrs)
            ...
            try:
                ...
                logger.debug("B: {} has fmt {} and failing on {}".format(
                    attr, fmt, value))
                mystr = str(value)
        return mystr
    ...
    def setData(self, index, value, role=QtCore.Qt.DisplayRole):
        ...
        if attr not in ["is_acceptable", "is_upper_limit", "user_flag"]:
            ...
            self.dataChanged.emit(
                ...
                self.createIndex(row, self.columnCount(None)))
            return value
...
class MeasurementSummaryTableModel(QtCore.QAbstractTableModel):
    ...
    def summarize(self):
        if self.session is None:
            ...
            self.summary = self.session.summarize_spectral_models(
                what_fe=self.what_fe)
    ...
    def update_summary(self, species=None):
        ...
        if len(row) != 1:
            logger.debug("Invalid species: {} ({})".format(
