...
from lesspayne.smh import Session, isoutils
...
class IsotopeDialog(QtGui.QDialog):
    ...
    def __init__(self,
        ...
        if isinstance(session, Session):
            if isinstance(orig_isotopes, dict):
                logger.warn(
                    "IsotopeDialog: Session already has isotopes, clobbering..."
                )
            ...
            if isinstance(needed_isotopes, dict):
                logger.warn(
                    "IsotopeDialog: Using session, clobbering needed_isotopes..."
                )
            needed_isotopes = None
        ...
        for key in orig_isotopes:
            if not isinstance(orig_isotopes[key], dict):
                raise Error(
                    "IsotopeDialog: orig_isotopes values are not dicts \n" +
                    str(orig_isotopes))
        ...
        if isinstance(needed_isotopes, dict):
            if isinstance(linelist, LineList):
                raise Error(
                    "IsotopeDialog: Cannot specify both needed_isotopes and linelist"
                )
        else:
        ...
        self.default_isotopes = [
            isoutils.load_isotope_data(x)
            for x in ["rproc", "sproc", "sneden", "asplund"]
        ]
        ...
        button_cancel = QtGui.QPushButton(
            "Restore original isotopes and exit (TODO)")
    ...
    def _use_data(self, i):
        ...
        try:
            self.table_model.set_element_with_isotopes(
                elem, self.default_isotopes[i])
    ...
    def validate(self):
        ...
        try:
            new_isotopes = isoutils.convert_array_to_isodict(
                self.table_model.tab)
...
class IsotopeModel(QtCore.QAbstractTableModel):
    ...
    def set_element_with_isotopes(self, elem, isotopes):
        if elem in isotopes:
            ...
            self.parent.text_box.appendMessage(
                "{} not in isotopes".format(elem))

    def setData(self, index, value, role):
        oldval = self.tab[index.row(), index.column()]
        self.tab[index.row(), index.column()] = value
        self.parent.text_box.appendMessage("Changed {} to {}".format(
            oldval, value))
        return True
...
if __name__ == "__main__":
    ...
    window = IsotopeDialog(
        session)  # orig_isotopes=orig_isotopes, linelist=ll)
    window.exec_()
