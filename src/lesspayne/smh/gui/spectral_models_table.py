...
class SpectralModelsTableViewBase(QtGui.QTableView):
    ...
    def contextMenuEvent(self, event):
        ...
        fit_models = menu.addAction("Fit selected model{}..".format(
            ["", "s"][N != 1]))
        menu.addSeparator()
        measure_models = menu.addAction("Measure selected model{}..".format(
            ["", "s"][N != 1]))
        menu.addSeparator()
...
class SpectralModelsTableModelBase(QtCore.QAbstractTableModel):
    ...
    def spectral_models(self):
        try:
            return self.parent.parent.session.metadata.get(
                "spectral_models", [])
        except AttributeError:
            # session is None
            return []
