            ...
            try:
                ...
                if attr == "equivalent_width":
                    value = "{0:.1f} +/- {1:.1f}".format(
                        1000 * value[0], 1000 * np.max(np.abs(value[1:])))
...
class SpectralModelsWidget(QtGui.QWidget):
    ...
    def figure_mouse_press(self, event):
        ...
        if event.dblclick:
            ...
            for i, (s, e) in enumerate(spectral_model.metadata["mask"][::-1]):
                if e >= event.xdata >= s:
                    ...
                    table_model.dataChanged.emit(
                        ...
                        table_model.createIndex(show_index.row(),
                                                table_model.columnCount(0)),
                    )
            ...
            self._mask_region_signal = (
                time(),
                self.mpl_figure.mpl_connect("motion_notify_event",
    ...
    def row_selected(self, *args):
        ...
        self.mpl_axis.set_xlim(lower_wavelength - pixel,
        ...
        for i, (start, end) in enumerate(model.metadata["mask"]):
            try:
                ...
                self._mpl_masked_regions.append(
                    self.mpl_axis.axvspan(np.nan,
                                          np.nan,
                                          facecolor="r",
                                          edgecolor="None",
                                          alpha=0.25))
            ...
            patch.set_xy([[start, -1e8], [start, +1e8], [end, +1e8],
                          [end, -1e8], [start, -1e8]])
