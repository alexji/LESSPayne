...
import lesspayne.smh as smh
from lesspayne.smh.gui.base import SMHSpecDisplay
from lesspayne.smh.gui.base import (
    MeasurementTableModelBase,
    MeasurementTableModelProxy,
    MeasurementTableView,
)
from lesspayne.smh.gui import base
from lesspayne.smh import utils
from lesspayne.smh.gui import mpl, style_utils
...
from lesspayne.smh.gui.spectral_models_table import (
    SpectralModelsTableViewBase,
    SpectralModelsFilterProxyModel,
    SpectralModelsTableModelBase,
)
from lesspayne.smh.gui.linelist_manager import TransitionsDialog
...
class ChemicalAbundancesTab(QtGui.QWidget):
    ...
    def __init__(self, parent):
        ...
        self.figure.mpl_connect("key_press_event",
                                self.key_press_automask_sigma)
        ## Stuff for extra synthesis
        self.extra_spec_1 = self.ax_spectrum.plot([np.nan], [np.nan],
                                                  ls="-",
                                                  color="#cea2fd",
                                                  lw=1.5,
                                                  zorder=9999)[0]
        self.extra_spec_2 = self.ax_spectrum.plot([np.nan], [np.nan],
                                                  ls="-",
                                                  color="#ffb07c",
                                                  lw=1.5,
                                                  zorder=9999)[0]
        ...
        self.filter_combo_box.currentIndexChanged.connect(
            self.filter_combo_box_changed)
    ...
    def _create_measurement_list(self):
        ...
        vbox, measurement_view, btn_filter, btn_refresh = (
            base.create_measurement_table_with_buttons(
                self,
                self.measurement_model,
                self.parent.session,
                callbacks_after_menu=[
        ...
        self.measurement_model.add_callback_after_setData(
            self.summarize_current_table)
    ...
    def _create_fitting_options_widget(self):
        ...
        def _create_line_in_hbox(parent,
                                 text,
                                 bot,
                                 top,
                                 dec,
        ...
        hbox, label, line = _create_line_in_hbox(self.tab_profile,
                                                 "View window", 0, 1000, 1)
        ...
        hbox, checkbox, label, combo = _create_checkcombo_in_hbox(
            self.tab_profile, "Use poly for cont.")
        ...
        hbox, checkbox, label, line = _create_checkline_in_hbox(
            self.tab_profile, "RV tol", 0, 100, 2)
        self.checkbox_vrad_tolerance = checkbox
        self.edit_vrad_tolerance = line
        vbox_lhs.addLayout(hbox)

        hbox, checkbox, label, line = _create_checkline_in_hbox(
            self.tab_profile, "WL tol", 0, 10, 2)
        ...
        self.checkbox_use_antimasks.setEnabled(
            False)  # Editable by shift clicking only
        ...
        vbox_rhs.addItem(
            QtGui.QSpacerItem(20, 20, QtGui.QSizePolicy.Minimum,
                              QtGui.QSizePolicy.Expanding))
        ...
        vbox_lhs.setSpacing(0)
        vbox_lhs.setContentsMargins(0, 0, 0, 0)
        hbox, label, line = _create_line_in_hbox(self.tab_synthesis,
                                                 "View window", -1000, 1000, 1)
        ...
        hbox, checkbox, label, combo = _create_checkcombo_in_hbox(
            self.tab_synthesis, "Use poly for cont.")
        ...
        self.synth_abund_table = SynthesisAbundanceTableView(
            self.tab_synthesis, )
        ...
        self.synth_abund_table.verticalHeader().setSectionResizeMode(
            QtGui.QHeaderView.Fixed)
        self.synth_abund_table.verticalHeader().setDefaultSectionSize(
            _ROWHEIGHT)
    ...
    def _connect_profile_signals(self):
        self.edit_view_window.textChanged.connect(self.update_edit_view_window)
        self.edit_fit_window.textChanged.connect(self.update_edit_fit_window)
        self.edit_fit_window.returnPressed.connect(self.fit_one)
        self.checkbox_continuum.stateChanged.connect(
            self.clicked_checkbox_continuum)
        self.checkbox_continuum.stateChanged.connect(self.fit_one)
        self.combo_continuum.currentIndexChanged.connect(
            self.update_continuum_order)
        self.combo_continuum.currentIndexChanged.connect(self.fit_one)
        self.checkbox_vrad_tolerance.stateChanged.connect(
            self.clicked_checkbox_vrad_tolerance)
        self.checkbox_vrad_tolerance.stateChanged.connect(self.fit_one)
        self.edit_vrad_tolerance.textChanged.connect(
            self.update_vrad_tolerance)
        self.edit_vrad_tolerance.returnPressed.connect(self.fit_one)
        self.combo_profile.currentIndexChanged.connect(
            self.update_combo_profile)
        self.combo_profile.currentIndexChanged.connect(self.fit_one)
        self.edit_detection_sigma.textChanged.connect(
            self.update_detection_sigma)
        self.edit_detection_sigma.returnPressed.connect(self.fit_one)
        self.edit_detection_pixels.textChanged.connect(
            self.update_detection_pixels)
        self.edit_detection_pixels.returnPressed.connect(self.fit_one)
        self.checkbox_use_central_weighting.stateChanged.connect(
            self.clicked_checkbox_use_central_weighting)
        self.checkbox_use_central_weighting.stateChanged.connect(self.fit_one)
        ...
        self.checkbox_wavelength_tolerance.stateChanged.connect(self.fit_one)
        self.edit_wavelength_tolerance.textChanged.connect(
            self.update_wavelength_tolerance)
        self.edit_wavelength_tolerance.returnPressed.connect(self.fit_one)
        ...
        self.btn_clear_masks.clicked.connect(self.clicked_btn_clear_masks)
        self._profile_signals = [
            ...
            (self.checkbox_continuum.stateChanged,
             self.clicked_checkbox_continuum),
            (self.checkbox_continuum.stateChanged, self.fit_one),
            (self.combo_continuum.currentIndexChanged,
             self.update_continuum_order),
            (self.combo_continuum.currentIndexChanged, self.fit_one),
            (
                self.checkbox_vrad_tolerance.stateChanged,
                self.clicked_checkbox_vrad_tolerance,
            ),
        ...
        #    self.synthDefaultAction)
        self.edit_manual_rv.textChanged.connect(self.update_manual_rv)
        ...
        #    self.synthDefaultAction)
        self.btn_synthesize.clicked.connect(self.synthesize_current_model)
        ...
        self.btn_fit_synth.clicked.connect(self.fit_one)
        self.btn_update_abund_table.clicked.connect(
            self.clicked_btn_update_abund_table)
        self.btn_clear_masks_2.clicked.connect(self.clicked_btn_clear_masks)
        self.btn_export_synth.clicked.connect(self.clicked_export_synthesis)

        self._synth_signals = [
            (self.edit_view_window_2.textChanged,
             self.update_edit_view_window_2),
            (self.edit_fit_window_2.textChanged,
             self.update_edit_fit_window_2),
            (self.checkbox_continuum_2.stateChanged,
             self.clicked_checkbox_continuum_2),
            # (self.checkbox_continuum_2.stateChanged,self.synthDefaultAction),
            (self.combo_continuum_2.currentIndexChanged,
             self.update_continuum_order_2),
            # (self.combo_continuum_2.currentIndexChanged,self.synthDefaultAction),
            (self.edit_manual_continuum.textChanged,
             self.update_edit_manual_continuum),
            # (self.edit_manual_continuum.returnPressed,self.synthDefaultAction),
            (
                self.checkbox_vrad_tolerance_2.stateChanged,
                self.clicked_checkbox_vrad_tolerance_2,
            ),
            # (self.checkbox_vrad_tolerance_2.stateChanged,self.synthDefaultAction),
            (self.edit_vrad_tolerance_2.textChanged,
             self.update_vrad_tolerance_2),
            ...
            (
                self.checkbox_upper_limit_2.stateChanged,
                self.clicked_checkbox_upper_limit_2,
            ),
            (self.btn_find_upper_limit.clicked,
             self.clicked_btn_find_upper_limit),
            (self.btn_fit_synth.clicked, self.fit_one),
            (self.btn_update_abund_table.clicked,
             self.clicked_btn_update_abund_table),
        ...
        if self._currently_plotted_element not in ["All", "", "None"]:
            try:
                self.measurement_model.delete_filter_function(
                    self._currently_plotted_element)
        ...
        if elem in [None, "", "All"]:
            ...
            def filter_function(model):
                if isinstance(model, ProfileFittingModel):
                    ...
                    return np.any(
                        [species in specie for specie in model.species])
    ...
    def summarize_current_table(self):
        ...
        if species not in summary_dict:
            logger.debug(
                "Cannot find species {} in summary_dict (keys={})".format(
    ...
    def fit_all_profiles(self):
        ...
        self.measurement_model.beginResetModel()
        num_unacceptable = 0
        for i, spectral_model in enumerate(
                self.full_measurement_model.spectral_models):
    ...
    def synthesize_current_model(self):
        ...
        try:
            ...
            if extra_abundances is not None:
                ...
                abundances1 = deepcopy(
                    spectral_model.metadata["rt_abundances"])
                ...
                for elem, abunddiff in extra_abundances[0].items():
                    ...
                    self.synth_abund_table_model.extra_abundances[elem][
                        0] = abunddiff
    ...
    def _check_for_spectral_models(self):
        for sm in self.parent.session.metadata.get("spectral_models", []):
            if sm.use_for_stellar_composition_inference:
                break
        else:
    ...
    def _get_selected_model(self, full_output=False):
        try:
            proxy_index = self.measurement_view.selectionModel().selectedRows(
            )[-1]
        except IndexError:
        ...
        try:
            ...
            if isinstance(selected_model, SpectralSynthesisModel):
                ...
                if selected_elem is not None:
                    ...
                    ii = np.logical_or(
                        transitions["elem1"] == selected_elem,
                ...
                label_rv = None
        # Update figure
        self.figure.update_spectrum_figure(
            redraw=redraw,
    ...
    def update_fitting_options(self):
        ...
        if isinstance(selected_model, ProfileFittingModel):
            ...
            self.edit_view_window.setText("{}".format(
                selected_model.metadata["window"] * 1.02))
            self.edit_fit_window.setText("{}".format(
                selected_model.metadata["window"]))
            ...
            vrad_tolerance = selected_model.metadata.get(
            ...
            tolerance = selected_model.metadata.get("wavelength_tolerance",
                                                    None)
            if tolerance is None:
                ...
                self.edit_wavelength_tolerance.setText("{}".format(tolerance))
            ...
            self.combo_profile.setCurrentIndex(
                ["gaussian", "lorentzian",
                 "voigt"].index(selected_model.metadata["profile"]))
        ...
        if isinstance(selected_model, SpectralSynthesisModel):
            ...
            self.edit_fit_window_2.setText("{}".format(
                selected_model.metadata["window"]))
            ...
            self.edit_manual_continuum.setText("{}".format(
                selected_model.metadata["manual_continuum"]))
            ...
            self.edit_manual_rv.setText("{:.4f}".format(
                selected_model.metadata["manual_rv"]))

            self.edit_initial_abundance_bound.setText("{}".format(
                selected_model.metadata["initial_abundance_bounds"]))
            ...
            self.edit_manual_smoothing.setText("{:.4f}".format(
                selected_model.metadata["manual_sigma_smooth"]))
    ...
    def update_continuum_order(self):
        ...
        self._get_selected_model().metadata["continuum_order"] = int(
            self.combo_continuum.currentText())
        return None
    ...
    def update_continuum_order_2(self):
        """The continuum order to use in model fitting was changed."""
        self._get_selected_model().metadata["continuum_order"] = int(
            self.combo_continuum_2.currentText())
        return None
    ...
    def update_initial_abundance_bound(self):
        ...
        try:
            ...
            ).metadata["initial_abundance_bounds"] = value
        return None
    ...
    def clicked_btn_find_upper_limit(self):
        ...
        try:
            ...
            return None
        upper_limit = spectral_model.find_upper_limit(sigma=sigma,
    ...
    def clicked_btn_update_abund_table(self):
        ...
        if selected_model is None:
            return None
        assert isinstance(selected_model,
                          SpectralSynthesisModel), selected_model
        summary_dict = self.parent.session.summarize_spectral_models(
            organize_by_element=True)
        ...
        for elem in selected_model.metadata["rt_abundances"]:
            try:
                selected_model.metadata["rt_abundances"][elem] = summary_dict[
                    elem][1]
            except KeyError:
        ...
        try:
            ...
            for i, elem in enumerate(selected_model.elements):
                try:
                    ...
                    logger.warn(
                        "No abundance found for {}, using nan".format(elem))
                    abund = np.nan
    ...
    def clicked_export_synthesis(self):
        ...
        synth_path, _ = QtGui.QFileDialog.getSaveFileName(
            self, caption="Enter synthesis output filename",
            dir="")  # , filter="*.txt")
        if not synth_path:
            return
        ## Ask for data output filename
        data_path, _ = QtGui.QFileDialog.getSaveFileName(
            self, caption="Enter data output filename",
            dir="")  # , filter="*.txt")
        if not data_path:
            return
        ## Ask for parameter output filename
        param_path, _ = QtGui.QFileDialog.getSaveFileName(
            self, caption="Enter parameter output filename",
            dir="")  # , filter="*.txt")
        if not param_path:
            return
        ## Ask for linelist output filename
        linelist_path, _ = QtGui.QFileDialog.getSaveFileName(
            self, caption="Enter linelist output filename",
            dir="")  # , filter="*.txt")
        if not param_path:
            return
        ## Export
        ...
        logger.info("Exported to {}, {}, {}, {}".format(
            synth_path, data_path, param_path, linelist_path))
        return
    ...
    def key_press_selectcheck(self, event):
        ...
        if event.key in ["f", "F"]:
            ...
            self.measurement_view.update_row(proxy_index.row())
            return None
        if event.key not in ["up", "down", "j", "J", "k", "K"]:
            return None
        try:
            proxy_index_row = (self.measurement_view.selectionModel().
                               selectedRows()[-1].row())
        except IndexError:
            return None
        if event.key in ["up", "j", "J"]:
            proxy_index_row -= 1
        if event.key in ["down", "k", "K"]:
            proxy_index_row += 1
    ...
    def key_press_automask_sigma(self, event):
        if event.key in ["9", "0", ",", "."]:
            ...
            if not isinstance(selected_model, ProfileFittingModel):
                return None
...
class SynthesisAbundanceTableModel(QtCore.QAbstractTableModel):
    ...
    def load_new_model(self, spectral_model):
        ...
        if spectral_model is not None:
            ...
            self.elem_order = dict(
                zip(
                    self.num_fit_elems + np.arange(len(elems)),
    ...
    def rowCount(self, parent):
        try:
            return self.num_fit_elems + len(
                self.spectral_model.metadata["rt_abundances"])
    ...
    def data(self, index, role):
        ...
        if elem in self.spectral_model.metadata["rt_abundances"]:
            ...
            key = "log_eps({})".format(elem)
            assert key in fitted_result[0], "{} {}".format(
                key, fitted_result[0])
        ...
        if index.column() == 1:
            ...
            return "{:.3f}".format(logeps - solar_composition(elem) -
                                   self.parent.FeH)
        elif (index.column() == 3) or (index.column() == 4):
            if (elem not in self.extra_abundances) or (
                    self.extra_abundances[elem][index.column() - 3] is None):
                return ""
            return "{:.3f}".format(self.extra_abundances[elem][index.column() -
                                                               3])
        return None
            ...
            try:
                ...
                for i, _elem in enumerate(self.spectral_model.elements):
                    ...
                    raise ValueError(elem + " " +
                                     str(self.spectral_model.elements))
