...
class Ui_MainWindow(QtGui.QMainWindow):
    ...
    def __init_menus__(self):
        ...
        ncap_summary_plot = QtGui.QAction(
            "&Ncap Summary Plot",
            self,
        ...
        # Export menu.
        self._menu_export_normalized_spectrum = QtGui.QAction(
        ...
        self._menu_export_unnormalized_spectrum = QtGui.QAction(
            "Unnormalized rest-frame spectrum",
            self,
            statusTip="Export a coadded rest-frame spectrum resampled "
            "onto a common wavelength mapping",
            triggered=self.export_unnormalized_spectrum,
        )
        self._menu_export_stitched_continuum = QtGui.QAction(
            "Stitched continuum",
            self,
        ...
        self._menu_print_abundance_table = QtGui.QAction(
            "Print abundance table",
            self,
            statusTip="",
            triggered=self.print_abundance_table,
        )
        self._menu_export_abundance_table = QtGui.QAction(
            "Export abundance table",
            self,
            statusTip="",
            triggered=self.export_abundance_table,
        )
        self._menu_export_spectral_model_measurements = QtGui.QAction(
            "Export spectral model measurements",
            self,
            statusTip="",
        ...
        self._default_statusbar_message = "Spectroscopy Made Harder v{} ({})".format(
    ...
    def open_session(self, path=None):
        ...
        if path is None or path is False:
            path, _ = QtGui.QFileDialog.getOpenFileName(
                self, caption="Select session", directory="", filter="*.smh")
            if not path:
                return
        print("We got:", path)
        ...
        if "rv" not in self.session.metadata:
            return None
        ...
        if ("rv_measured" not in self.session.metadata["rv"]
                and "rv_applied" not in self.session.metadata["rv"]):
            return None

        self.normalization_tab._populate_widgets()
        print("populated normalization widgets")
        if "normalization" not in self.session.metadata:
            return None
        ...
        if "continuum" not in self.session.metadata["normalization"]:
            return None
    ...
    def save_session_as(self, path=None):
        ...
        if path is None or path is False:
            path, _ = QtGui.QFileDialog.getSaveFileName(
                self, caption="Enter filename", directory="", filter="*.smh")
            if not path:
                return
    ...
    def show_balmer_line_dialog(self):
        ...
        dialog = BalmerLineFittingDialog(spectra,
                                         observed_spectra_labels=labels,
    ...
    def export_normalized_spectrum(self):
        ...
        path, _ = QtGui.QFileDialog.getSaveFileName(
            self,
            caption="Enter normalized rest frame spectrum filename",
            directory="",
            filter="",
        )
        if not path:
    ...
    def export_unnormalized_spectrum(self):
        ...
        path, _ = QtGui.QFileDialog.getSaveFileName(
            self,
            caption="Enter unnormalized rest frame spectrum filename",
            directory="",
            filter="",
        )
        if not path:
    ...
    def export_stitched_continuum(self):
        ...
        if self.session is None:
            return
        path, _ = QtGui.QFileDialog.getSaveFileName(
            self, caption="Enter continuum filename", directory="", filter="")
        if not path:
            return
    ...
    def print_abundance_table(self):
        ...
        for key in keys:
            ...
            print("{:7.1f} {:3} {:6.2f} {:4.2f} {:5.2f} {:6.2f}".format(
                key, num_models, logeps, stdev, XH, XFe))
    ...
    def export_abundance_table(self):
        ...
        path, _ = QtGui.QFileDialog.getSaveFileName(
            self,
            caption="Enter abundance table filename",
            directory="",
            filter="")
        if not path:
            return
    ...
    def export_spectral_model_measurements(self):
        ...
        path, _ = QtGui.QFileDialog.getSaveFileName(
            self,
            caption="Enter spectral model measurements filename",
            directory="",
            filter="",
        )
        if not path:
    ...
    def comparison_spectrum_dialog(self):
        ...
        """
        path, _ = QtGui.QFileDialog.getOpenFileName(
            self, caption="Pick comparison spectrum", directory="", filter="")
        if not path:
            return
        spectrum = specutils.Spectrum1D.read(path)
        self.stellar_parameters_tab.specfig.update_comparison_spectrum(
            spectrum)
        self.chemical_abundances_tab.figure.update_comparison_spectrum(
            spectrum)
        return None
    ...
    def __init_ui__(self):
        ...
        self.stellar_parameters_tab = stellar_parameters.StellarParametersTab(
            self)
        self.tabs.addTab(self.stellar_parameters_tab, "Stellar parameters")

        # Create chemical abundances tab
        self.chemical_abundances_tab = chemical_abundances.ChemicalAbundancesTab(
            self)
        self.tabs.addTab(self.chemical_abundances_tab, "Line Measurements")

        # Create review tab
        self.review_tab = review.ReviewTab(self)
