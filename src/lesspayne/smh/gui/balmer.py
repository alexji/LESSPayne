...
from lesspayne.smh.balmer import BalmerLineModel
...
class Worker(QtCore.QThread):
    ...
    def run(self):
        ...
        process = mp.Process(
            target=_BALMER_LINE_MODEL.infer,
            args=(spectrum, ),
...
class BalmerLineFittingDialog(QtGui.QDialog):
        ...
        **kwargs,
        ...
        if isinstance(observed_spectra, Spectrum1D):
            observed_spectra = [observed_spectra]
                ...
                observed_spectra, (list, tuple, np.ndarray)) or not all(
                    [isinstance(s, Spectrum1D) for s in observed_spectra]):
            raise TypeError(
                "observed spectra must be a Spectrum1D or a list-like of Spectrum1D"
            )

        if observed_spectra_labels is not None:
            if len(observed_spectra_labels) != len(observed_spectra):
                raise ValueError(
                    "number of observed spectra does not match the"
                    " number of observed spectra labels ({} != {})".format(
                        len(observed_spectra_labels), len(observed_spectra)))

            if len(set(observed_spectra_labels)) < len(
        ...
        if session is not None:
            ...
            if session_settings:
                ...
                # Update the masks.
                self.p1_figure.dragged_masks = [] + session_settings.get(
                    "masks", [])
    ...
    def _add_pane_1(self):
        ...
        for name, wavelength in zip(self.__balmer_line_wavelengths,
                                    self.__balmer_line_names):
            self.combo_balmer_line_selected.addItem("{} ({} Å)".format(
                name, wavelength))
    ...
    def save_to_session(self):
        ...
        while True:
            ...
            result_name, ok = QtGui.QInputDialog.getText(
                self,
                "Result name",
                "Specify a name for this result:",
                text=default_text,
            )

            if not ok or result_name not in self.session.metadata[
                    "balmer_line_fits"]:
                break
    ...
    def show_third_pane(self):
        ...
        global _BALMER_LINE_MODEL
        _BALMER_LINE_MODEL = BalmerLineModel(
            glob(model_wildmask),
    ...
    def populate_widgets_in_pane4(self):
        ...
        _BALMER_LINE_MODEL.plot_projection(
            spectrum,
    ...
    def draw_grid_points(self):
        ...
        for index in indices:
            ...
            for j, feh in enumerate(model.stellar_parameters[match, 2]):
                offset_x = r_x * np.cos(j * theta / 180.0 * np.pi + np.pi / 2)
                offset_y = r_y * np.sin(j * theta / 180.0 * np.pi + np.pi / 2)
    ...
    def updated_balmer_line_selected(self):
        ...
        for j, idx in enumerate(
                self._balmer_line_indices[selected_balmer_index]):
        ...
        self.combo_spectrum_selected.setCurrentIndex(selected_spectrum_index
                                                     or 0)
    ...
    def updated_spectrum_selected(self):
        ...
        limits = (limits[0] - 0.05 * np.ptp(limits),
                  limits[1] + 0.05 * np.ptp(limits))
        ...
        wavelength = self.__balmer_line_wavelengths[
            self.combo_balmer_line_selected.currentIndex()]
        ax.lines[1].set_data(np.array([[wavelength, wavelength], [-1e8,
                                                                  +1e8]]))
...
class BalmerLineModelParametersTableModel(QtCore.QAbstractTableModel):
    ...
    def __init__(self, parent, *args):
        super(BalmerLineModelParametersTableModel,
              self).__init__(parent, *args)
        self.parent = parent
...
class BalmerLineOptionsTableModel(QtCore.QAbstractTableModel):
    ...
    def setData(self, index, value, role):
        if index.column() == 0:
            ...
            if index.row() == 2:
                ...
                if value:
                    N, is_ok = QtGui.QInputDialog.getItem(
                        None,
                        "Continuum order",
                        "Specify continuum order:",
                        [
                            "{:.0f}".format(i)
            ...
            try:
                ...
                if (index.column() == 1 and bounds[1] is not None
                        and value >= bounds[1]) or (index.column() == 2
                ...
                self.parent.metadata["bounds"][parameter][index.column() -
                                                          1] = value
    ...
    def data(self, index, role):
        ...
        elif index.column() == 0 and index.row(
        ) >= 3 and role == QtCore.Qt.DisplayRole:
            return ""
...
if __name__ == "__main__":
    ...
    window = BalmerLineFittingDialog(
        spectra,
        observed_spectra_labels=[
            "Order {}".format(i) for i in range(1, len(spectra))
