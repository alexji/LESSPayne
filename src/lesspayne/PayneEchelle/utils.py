...
def read_in_neural_network():
    ...
    path = pkg_resources.resource_filename(
        "lesspayne", "data/NN_normalized_spectra_float16_fixwave.npz")
...
def read_default_model_mask(wavelength_payne=None):
    ...
    theory_mask_path = pkg_resources.resource_filename(
        "lesspayne", "PayneEchelle/other_data/theory_mask.txt")
    theory_mask = np.loadtxt(theory_mask_path)
    for wmin, wmax in theory_mask:
        assert wmin < wmax, (wmin, wmax)
        errors_payne[(wavelength_payne >= wmin)
                     & (wavelength_payne <= wmax)] = 999.0
    return errors_payne
...
def read_gaia_eso_benchmark_mask(Nmask=11, thresh=0.8):
    ...
    path = pkg_resources.resource_filename(
        "lesspayne", "data/gaia_eso_benchmark_mask_info.pkl")
...
def read_in_example():
    ...
    path = pkg_resources.resource_filename(
        "lesspayne", "data/spectra/2M06062375-0639010_red_multi.fits")
...
def read_in_blaze_spectrum():
    ...
    path = pkg_resources.resource_filename("lesspayne",
                                           "data/spectra/Hot_Star_HR718.fits")
    wavelength_blaze, spectrum_blaze, spectrum_err_blaze = read_carpy_fits(
        path)
...
def match_blaze_to_spectrum(wavelength, spectrum, wavelength_blaze,
                            spectrum_blaze):
    """
    ...
    for i in range(wavelength.shape[0]):
        f_blaze = interpolate.interp1d(wavelength_blaze[i, :],
                                       spectrum_blaze[i, :])
...
def mask_telluric_region(spectrum_err,
                         spectrum_blaze,
                         smooth_length=30,
                         threshold=0.9):
    """
    ...
    for j in range(spectrum_blaze.shape[0]):
        for i in range(spectrum_blaze[j, :].size - smooth_length):
            if np.min(
                    spectrum_blaze[j,
                                   i:i + smooth_length]) < threshold * np.max(
                                       spectrum_blaze[j, i:i + smooth_length]):
                spectrum_err[j, i:i + smooth_length] = 999.0
    return spectrum_err
...
def cut_wavelength(wavelength,
    ...
    ii_good = (np.sum(
        (wavelength > wavelength_min) & (wavelength < wavelength_max),
        axis=1) == wavelength.shape[1])
    print("Keeping {}/{} orders between {}-{}".format(ii_good.sum(),
                                                      len(ii_good),
                                                      wavelength_min,
                                                      wavelength_max))
    return wavelength[ii_good, :], spectrum[ii_good, :], spectrum_err[
        ii_good, :]
...
def mask_wavelength_regions(wavelength, spectrum_err, mask_list):
    ...
    for wmin, wmax in mask_list:
        assert wmin < wmax
        spectrum_err[(wavelength > wmin) & (wavelength < wmax)] = 999.0
    return spectrum_err
...
def scale_spectrum_by_median(spectrum, spectrum_err):
    ...
    for i in range(spectrum.shape[0]):
        scale_factor = 1.0 / np.median(spectrum[i, :])
...
def whitten_wavelength(wavelength):
    ...
    for k in range(wavelength.shape[0]):
        ...
        wavelength_normalized[k, :] = (wavelength[k, :] -
                                       mean_wave) / mean_wave
