...
def check_satisfies_tolerance(final_parameters_result, total_tolerance,
                              individual_tolerances):
    acquired_total_tolerance = np.sum(np.array(final_parameters_result)**2)
    return acquired_total_tolerance < total_tolerance and (
        individual_tolerances is None or np.all(
            np.less_equal(np.abs(final_parameters_result),
                          individual_tolerances)))
...
def stellar_optimization(func):
    ...
    def _decorator(request, *args, **kwargs):
        ...
        if len(previously_sampled_points) > 0:
            ...
            try:
                sampled_before = (
                    previously_sampled_points[:, :4] == this_point).all(axis=1)
            except:
                sampled_before = (
                    previously_sampled_points[:, :4] == this_point).all()
            ...
            if np.any(sampled_before):
                ...
                logger.debug(
                    "Sampled these stellar parameters already, so we're just returning those values."
                )
        ...
        if total_tolerance >= acquired_total_tolerance and (
                individual_tolerances is None or np.all(
                    np.less_equal(np.abs(response), individual_tolerances))):
...
def optimize_stellar_parameters(
    initial_guess,
    transitions,
    EWs=None,
    ...
    def minimisation_function(stellar_parameters, *args):
        ...
        logger.info(
            ...
            " yields sum {5:.1e}:\n\t\t\t[{6:.1e}, {7:.1e}, {8:.1e}, {9:.1e}]".
            format(teff, vt, logg, feh, alpha, acquired_total_tolerance,
                   *results))
        return results
    ...
    for i in range(1, 1 + max_attempts):
        ...
        try:
            results = fsolve(
                ...
                solver_guess,
                args=args,
                fprime=utils.approximate_stellar_jacobian,
                col_deriv=1,
                epsfcn=0,
                xtol=1e-10,
                full_output=1,
            ...
            final_parameters[0] = int(np.round(
                final_parameters[0]))  # Effective temperature
            ...
            acquired_total_tolerance = sum(
                pow(np.array(final_parameters_result), 2))
            return (
                True,
                initial_guess,
                num_moog_iterations,
            ...
            if total_tolerance >= acquired_total_tolerance and (
                ...
                solver_guess = [
                    np.random.uniform(*parameter_ranges[parameter])
                    for parameter in ["teff", "vt", "logg", "[Fe/H]"]
                ]
...
def optimize_stellar_parameters_2(
        initial_guess,
        transitions,
        EWs=None,
        alphafe=0.4,
    ...
    finite = (transitions["equivalent_width"]
              > min_eqw) & (transitions["equivalent_width"] < max_eqw)
    if np.sum(finite) != len(finite):
        warnings.warn(
            "Only {}/{} eqw are finite or within {} < eqw < {}".format(
    ...
    if np.sum(idx_expot) < min_lines:
        raise RuntimeError(
            "Fewer than {} lines for expot species {:.1f}".format(
                min_lines, expot_balance_species))
    idx_rew = (transitions["species"] == rew_balance_species) & finite
    if np.sum(idx_rew) < min_lines:
        raise RuntimeError("Fewer than {} lines for rew species {:.1f}".format(
            min_lines, rew_balance_species))
    idx_I = (transitions["species"] == ionization_balance_species_1) & finite
    if np.sum(idx_I) < min_lines:
        raise RuntimeError(
            "Fewer than {} lines for Ion I species {:.1f}".format(
                min_lines, ionization_balance_species_1))
    idx_II = (transitions["species"] == ionization_balance_species_2) & finite
    if np.sum(idx_II) < min_lines:
        raise RuntimeError(
            "Fewer than {} lines for Ion II species {:.1f}".format(
                min_lines, ionization_balance_species_2))
    idx_MH = (transitions["species"] == mh_species) & finite

    unique_species = np.unique([
        expot_balance_species,
        rew_balance_species,
        ionization_balance_species_1,
        ionization_balance_species_2,
        mh_species,
    ])
    ...
    def minimisation_function(stellar_parameters, *args):
        ...
        if (teff < parameter_ranges["teff"][0]
                or teff > parameter_ranges["teff"][1]
                or logg < parameter_ranges["logg"][0]
                or logg > parameter_ranges["logg"][1]
                or vt < parameter_ranges["vt"][0]
                or vt > parameter_ranges["vt"][1]
                or feh < parameter_ranges["[Fe/H]"][0]
                or feh > parameter_ranges["[Fe/H]"][1] or np.isnan(teff)
                or np.isnan(logg) or np.isnan(vt) or np.isnan(feh)):
            # return np.array([np.nan, np.nan, np.nan, np.nan])
    ...
    for i in range(1, 1 + max_attempts):
        ...
        try:
            results = fsolve(
                ...
                solver_guess,
                args=args,
                fprime=utils.approximate_stellar_jacobian_2,
                col_deriv=1,
                epsfcn=0,
                xtol=1e-10,
                full_output=1,
        ...
        ## Check tolerance
        tolerance_achieved = check_satisfies_tolerance(final_parameters_result,
                                                       total_tolerance,
                                                       individual_tolerances)
        ...
        logger.info("Iteration {}/{}: clipping {} lines".format(
            i, max_attempts, to_clip.sum()))
        logger.info("Iteration {}/{}: {}".format(i, max_attempts,
                                                 np.where(to_clip)[0]))
        valid[valid] = np.logical_not(to_clip[valid])
        logger.info("Iteration {}/{}: now {} lines".format(
            i, max_attempts, valid.sum()))
        # Propagate to idx
        ...
        if to_clip.sum() == 0 and not tolerance_achieved:
            ...
            solver_guess = [
                np.random.uniform(*parameter_ranges[parameter])
                for parameter in ["teff", "logg", "vt", "[Fe/H]"]
            ]
            logger.info(
                "No lines to clip and tolerance not achieved: trying random restart at {}"
                .format(solver_guess))
    ...
    t_elapsed = time.time() - start
    logger.info("Took {:.1f}s, used {}/{} lines; Tol. achieved? {}".format(
        t_elapsed, valid.sum(), len(valid), tolerance_achieved))
...
def optimize_feh(
    ...
    initial_guess order : [teff, vt, logg, feh]
    optimize_vt_to_fe2 (bool) :: specificies whether to optimize REW slope (vt) to [Fe II/H] or [Fe I/H].
    """
    ...
    logger.info(
        f"Holding {nconst} parameter{'s' if nconst > 1 else ''} constant: " +
        ", ".join([
            plist[p] + "=" + str(initial_guess[p])
            for p in range(len(plist)) if not params_to_optimize[p]
        ]))

    # if we aren't optimizing the parameter, lock it to the initial guess (our value)
    parameter_ranges = {
        "Teff": (3500, 7000),
        "vt": (0.0, 4.0),
        "logg": (0, 5),
    ...
    for i in range(4):
        if not params_to_optimize[i]:
            parameter_ranges[plist[i]] = (initial_guess[i], initial_guess[i])
    ...
    def minimisation_function(stellar_parameters, *args):
        ...
        dFe = np.mean(abundances[idx_I]) - np.mean(
            abundances[idx_II])  # ionization balance (logg)
        dM = np.mean(abundances[idx_II]) - (feh + solar_composition("Fe")
                                            )  # metallicity difference (Fe/H)

        results = np.array([dAdchi, dAdREW, 0.1 * dFe, 0.1 * dM])
        acquired_total_tolerance = np.sum(results**2)

        # TODO: allow for a specific tolerance for dM = [M/H] - [Fe / H]
    ...
    for i in range(1, 1 + max_attempts):
        ...
        try:
            results = fsolve(
                ...
                solver_guess,
                args=args,
                fprime=utils.approximate_feh_jacobian,
                col_deriv=1,
                epsfcn=0,
                xtol=1e-10,
                full_output=1,
            ...
            sampled_points_tols = np.array(
                sampled_points)[:, 4:] * params_to_optimize
            ...
            final_parameters = (results[0] * params_to_optimize) + (
                initial_guess * (-1 * params_to_optimize + 1))
            final_parameters[0] = int(np.round(
                final_parameters[0]))  # Effective temperature
            final_parameters_result = results[1]["fvec"]
            num_moog_iterations = len(sampled_points)
